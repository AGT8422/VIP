...
class RecipeController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('manufacturing::recipe.index')->with(compact("currency_details"));
    }
    ...
    public function getIngredientRow($variation_id)
    {
        ...
        $product_cost = \App\Product::product_cost($variation_id);
        ...
        return view('manufacturing::recipe.ingredient_row', compact('ingredient', 'product_cost','row_index', 'ig_index', 'sort_order'));
    }
    ...
    public function addIngredients()
    {
        ...
        if (!empty($recipe) || (empty($recipe) && !empty($copy_recipe))) {
            ...
            foreach ($ingredients_obj as $ingredient) {
                ...
                $product_cost = \App\Product::product_cost($ingredient->variation->product->id);
               
                $temp = [
                    'id' => $ingredient->variation->id,
                    'dpp_inc_tax' => $product_cost,
        ...
        return view('manufacturing::recipe.add_ingredients', compact('variation', 'product_cost','ingredients', 'recipe', 'unit_html', 'currency_details'));
    }
    ...
    public function getRecipeDetails()
    {
        ...
        $old_store    = request()->input('old_store');
        ...
        $ingredient_table = view('manufacturing::recipe.ingredients_for_production', compact('ingredients', 'recipe','old_store','store_id', 'pos_settings', 'manufacturing_settings'))->render();
        
        $pro        = \App\Product::find($recipe->variation->product->id); 
        ...
        if (!empty($sub_units)) {
            ...
            foreach ($sub_units as $key => $value) {
                $unit_html .= '<option value="' . $key . '" data-multiplier="' . $value['multiplier'] . '" data-unit_name="' . $value['name'] . '" data-check_price="' . $value['check_price'] . '"  data-price="' . $value['price'] . '" ';
    ...
    public function destroy($id)
    {
        ...
        try {
            $recipe    = MfgRecipe::find($id);
           
            $childs    = $recipe->ingredients->pluck("id");
            foreach($childs as $id){
                    $mfg_item = MfgRecipeIngredient::find($id);
                    $mfg_item->delete();
            }
            $recipe->delete();
