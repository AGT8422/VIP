...
class Contact extends Authenticatable
{
    ...
    */
    ...
    {
        return $this->hasMany('App\Transaction','contact_id');
    }
    ...
    // Osama
    public static function suppliers($business_id=null)
    {
        $business_id = ($business_id!=null)?$business_id:request()->session()->get('user.business_id');
        $allData     = Contact::where('business_id',$business_id)->where('type','supplier')->orWhere('type','both')->get();
        $arr         = [];
    ...
    public static function customers()
    {
        ...
        $allData     = Contact::where('business_id',$business_id)->where('type','customer')->orWhere('type','both')->get();
        $arr         = [];
        foreach ($allData  as $data) {
            $arr[$data->id] =  $data->name . "( " . $data->contact_id . " )";
        }
    ...
    public static function add_account($id,$business_id=null,$account_name=null)
    {
        ...
        if (empty($account)) {
            $type =  Contact::add_type('both',$business_id); 
            switch ($data->type) {
                case 'customer':
                    if($account_name != null){
                        $type =  Contact::add_type($account_name,$business_id); 
                    }else{
                        $type =  Contact::add_type('Customers',$business_id); 
                    }
                    break;
                case 'supplier':
                    $type =  Contact::add_type('Suppliers',$business_id); 
                    break;
            }
            $account                  =  new \App\Account;
            $account->contact_id      =  $id;
            $account->name            =  $data->first_name;
            $account->account_number  =  $data->contact_id;
    ...
    public static function add_type($type,$business_id=null)
    {
        $business_id   = ($business_id != null)?$business_id:request()->session()->get('user.business_id');
        $account_type  = \App\AccountType::where('business_id',$business_id)->where('name',$type)->first();
