...
class ICheckController extends Controller
{
    public function index($id,Request $request)
    {
        \App::setlocale('en');
        $invoice =   Check::find($id);
        $pattern = null;
        $entry   =   Entry::where("check_id",$id)->first(); 
        ...
        if ($invoice) {
            ...
            $allData =  Check::where('business_id',$invoice->business_id)->get();
            $arr = [
                'layout'=>$layout,
                'invoice'=>$invoice,
                'allData'=>$allData,
            ...
            $pdf = PDF::loadView('pdf.Check.Cheque_in', $arr);
             
            return $pdf->stream('Cheque_out.pdf');
        }else {
           return '<h1> Sorry This Cheque Out Not Found </h1>'; 
        }
