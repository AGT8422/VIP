...
class JuVoucherController extends Controller
{
    public function index($id,Request $request)
    {
            \App::setlocale('en');
            $invoice =  DailyPayment::find($id);
            $pattern =  null;
            $entry   =  Entry::where("journal_voucher_id",$id)->first(); 
            if ($invoice) {
                ...
                $allData =  DailyPayment::where('business_id',$invoice->business_id)->get();
                $items   =  DailyPaymentItem::where('daily_payment_id',$invoice->id)->get();
                $arr = [
                    'layout'=>$layout,
                    'invoice'=>$invoice,
                    'allData'=>$allData,
                ...
                return $pdf->stream('J_voucher.pdf');
        }else {
           return '<h1> Sorry This Journal Voucher Not Found </h1>'; 
        }
