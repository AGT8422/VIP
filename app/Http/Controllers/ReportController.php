...
class ReportController extends Controller
{
    ...
    public function getProfitLoss(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            if($until_date != null){ $end_date =  $until_date ;}
            $reportSetting  =  \App\Models\ReportSetting::select("*")->first();

            $data = $this->transactionUtil->getProfitLossDetails($business_id, $location_id, $start_date, $end_date,null,$reportSetting);
            
            $closing              = \App\Product::closing_stock($business_id);
            
            // $opening_stock_filter = \App\Product::closing_stock_filter($business_id, $start_date, $end_date);
             
            return view('report.partials.profit_loss_details', compact('data','closing'))->render();
        }

        $business_locations = BusinessLocation::forDropdown($business_id, true);
        return view('report.profit_loss', compact('business_locations','currency_details'));
    }
  /**
     * Shows Report Setting of a business
     *
     * @return \Illuminate\Http\Response
     */
    public function reportSetting(Request $request)
    {
        ...
        $business_id = $request->session()->get('user.business_id');
        $title       = trans("lang_v1.setting_report");
        $accounts    = [];
        $account     = \App\AccountType::where("business_id",$business_id)->get();
        foreach($account as $i){
            $accounts[$i->id] =  $i->name . " ||  (" . $i->code.") "; 
        }
        $data  =    \App\Models\ReportSetting::select("*")->first(); 

        return view('report.partials.settings',compact("title","data","accounts"));
    }
    /**
     * Shows Report Setting of a business
     *
     * @return \Illuminate\Http\Response
     */
    public function reportSettingSave(Request $request)
    {
        if (!auth()->user()->can('profit_loss_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        try{
            ...
            $data  =    \App\Models\ReportSetting::select("*")->first(); 
            if(empty($data)){
                $datas  =  new  \App\Models\ReportSetting ; 
                $datas->sale   = $request->sale_account;
                $datas->sale_return   = $request->return_sale_account;
                $datas->sale_addtional_cost   = $request->sale_additional_cost;
                $datas->sale_discount   = $request->sale_discount;
                $datas->purchase   = $request->purchase_account;
                $datas->purchase_return   = $request->return_purchase_account;
                $datas->purchase_addtional_cost   = $request->purchase_additional_cost;
                $datas->purchase_discount   = $request->purchase_discount;
                $datas->purchase_opening_stock   = $request->purchase_opening_stock;
                $datas->purchase_closing_stock   = $request->purchase_closing_stock;
                $datas->expense   = $request->expense_account;
                $datas->revenue   = $request->revenue_account;
                $datas->save();
            }else{
                $data->sale   = $request->sale_account;
                $data->sale_return   = $request->return_sale_account;
                $data->sale_addtional_cost   = $request->sale_additional_cost;
                $data->sale_discount   = $request->sale_discount;
                $data->purchase   = $request->purchase_account;
                $data->purchase_return   = $request->return_purchase_account;
                $data->purchase_addtional_cost   = $request->purchase_additional_cost;
                $data->purchase_discount   = $request->purchase_discount;
                $data->purchase_opening_stock   = $request->purchase_opening_stock;
                $data->purchase_closing_stock   = $request->purchase_closing_stock;
                $data->expense   = $request->expense_account;
                $data->revenue   = $request->revenue_account;
                $data->update();
            }
            $output  = [
                "success" => 1,
                "msg" => __('lang_v1.add_success'),
            ];
        }catch(\Exception $e){
            \DB::rollBack();
    ...
    public function getPurchaseSell(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $location_id      = $request->get('location_id');
            $reportSetting    =  \App\Models\ReportSetting::select("*")->first();
            $purchase_details = $this->transactionUtil->getPurchaseTotals($business_id, $start_date, $end_date, $location_id,null,$reportSetting);
        ...
        return view('report.purchase_sell')
                    ->with(compact('business_locations','currency_details'));
    }
    ...
    public function getCustomerSuppliers(Request $request)
    {
        ...
        ->with(compact('customer_group', 'types','currency_details'));
    }
    ...
    {
        if (!auth()->user()->can('stock_report.view') && !auth()->user()->can('ReadOnly.views')&& !auth()->user()->can('manufuctoring.views')&& !auth()->user()->can('admin_supervisor.views')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        if ($request->ajax()) {
            ...
            ->addColumn(
                'action', function ($row) use ($selling_price_group_count) {
                        $html =
                        '<div class="btn-group"><button type="button" id="view_s" class="btn btn-info dropdown-toggle btn-xs" data-toggle="dropdown" aria-expanded="false">'. __("messages.actions") . '<span class="caret"></span><span class="sr-only">Toggle Dropdown</span></button><ul class="dropdown-menu dropdown-menu-left" role="menu">';
                        if (!auth()->user()->can('ReadOnly.views')) {
                        // $html .=
                        // '<li><a id="view_s" href="' . action('LabelsController@show') . '?product_id=' . $row->product_id . '" data-toggle="tooltip" title="' . __('lang_v1.label_help') . '"><i class="fa fa-barcode"></i> ' . __('barcode.labels') . '</a></li> ';                   
                        }
                        if (auth()->user()->can('ReadOnly.views')) {
                            $html .=
                            '<li><a id="view_s" href="' . action('ProductController@view', [$row->product_id]) . '" class="view-product"><i class="fa fa-eye"></i> ' . __("messages.view") . '</a></li>';
                        }

                        if (auth()->user()->can('product.update')) {
                            $html .=
                            '<li><a id="view_s" href="' . action('ProductController@edit', [$row->product_id]) . '"><i class="glyphicon glyphicon-edit"></i> ' . __("messages.edit") . '</a></li>';
                        }

                        if ( request()->session()->get("user.id") == 1 || request()->session()->get("user.id") == 7 || request()->session()->get("user.id") == 8) {
                            $html .=
                            '<li><a id="view_s" href="' . action('ProductController@destroy', [$row->product_id]) . '" class="delete-product"><i class="fa fa-trash"></i> ' . __("messages.delete") . '</a></li>';
                        }


                        if (auth()->user()->can('product.update')) {
                            $html .=
                                '<li><a id="view_s" href="' . action('ProductController@addbarcode', [$row->product_id]) . '" ><i class="fa fa-plus-circle"></i> ' . __("messages.morebarcode") . '</a></li>';
                        }

                        if (auth()->user()->can('ReadOnly.views')) {
                            $html .=
                            '<li><a href="' . action('ItemMoveController@index', [$row->product_id]) . '"><i class="fas fa-history"></i> ' . __("lang_v1.product_stock_history") . '</a></li>';
                        }
                        if ($row->is_inactive == 1) {
                            $html .=
                            '<li><a id="view_s" href="' . action('ProductController@activate', [$row->product_id]) . '" class="activate-product"><i class="fas fa-check-circle"></i> ' . __("lang_v1.reactivate") . '</a></li>';
                        }
                        ...
                        if ($row->enable_stock == 1 && auth()->user()->can('product.opening_stock')) {
                            $html .=
                            '<li><a id="view_s" href="#" data-href="' . action('OpeningStockController@add', ['product_id' => $row->product_id]) . '" class="add-opening-stock"><i class="fa fa-database"></i> ' . __("lang_v1.add_edit_opening_stock") . '</a></li>';
                        }

                        if (auth()->user()->can('product.view')) {
                            $html .=
                            '<li><a id="view_s" href="' . action('ProductController@productStockHistory', [$row->product_id]) . '"><i class="fas fa-history"></i> ' . __("lang_v1.product_stock_history") . '</a></li>';
                        }
                        ...
                        if (auth()->user()->can('product.create')) {
                            ...
                            if ($selling_price_group_count > 0) {
                                $html .=
                                '<li><a id="view_s" href="' . action('ProductController@addSellingPrices', [$row->product_id]) . '"><i class="fas fa-money-bill-alt"></i> ' . __("lang_v1.add_selling_price_group_prices") . '</a></li>';
                            }

                            $html .=
                                '<li><a id="view_s" href="' . action('ProductController@create', ["d" => $row->product_id]) . '"><i class="fa fa-copy"></i> ' . __("lang_v1.duplicate_product") . '</a></li>';
                        }

                        if (!empty($row->media->first())) {

                            $html .=
                                '<li><a id="view_s" href="' . $row->media->first()->display_url . '" download="'.$row->media->first()->display_name.'"><i class="fas fa-download"></i> ' . __("lang_v1.product_brochure") . '</a></li>';
                        }
            ...
            ->editColumn('unit_price', function ($row) use ($allowed_selling_price_group) {
                ...
                if (auth()->user()->can('access_default_selling_price')) {
                    $prc = $row->unit_price;
                    $item_move = \App\Models\ItemMove::where("product_id",$row->product_id)->orderByRaw('ISNULL(date), date desc, created_at desc')->orderBy("id","desc")->orderBy("order_id","desc")->first();
                    if(!empty($item_move)){
                        $prc = $item_move->unit_cost;
                    }
                ...
                if ($allowed_selling_price_group) {
                    $html .= ' <button type="button" id="view_s" class="btn btn-primary btn-xs btn-modal no-print" data-container=".view_modal" data-href="' . action('ProductController@viewGroupPrice', [$row->product_id]) .'">' . __('lang_v1.view_group_prices') . '</button>';
                }
            ...
            ->editColumn('stock', function ($row) {
                if ($row->enable_stock) {
                    ...
                    $warehouse = WarehouseInfo::where("product_id",$row->product_id)
                            ->select(DB::raw("SUM(product_qty) as stock"))->first();

                    $stock = $row->stock ? $row->stock : 0 ;

                    $html = '<span data-is_quantity="true" class="current_stock display_currency" data-orig-value="' . (float)$warehouse->stock . '" data-unit="' . $row->unit . '" data-currency_symbol=false > ' . (float)$warehouse->stock . '</span>' . ' ' . $row->unit ;
                    
                    if(  $warehouse->stock != null ){
                        $html .= ' <button type="button" class="btn btn-primary btn-xs btn-modal no-print" id="view_s" data-container=".view_modal" data-href="' . action('ProductController@viewStock', [$row->product_id]) .'">' . __('lang_v1.view_Stock') . '</button>';
                    }
                    return  $html;
                } else {
                    return '--';
            ...
            ->addColumn('total_recieve', function ($row) {
                $view = "";
                $data  = PurchaseLine::OrderBy('id','desc')->where('product_id',$row->product_id)->whereHas('transaction',function($query){
                                        $query->where('type','purchase');
                                        $query->where('status','!=','received');
                                    })->get()->sum('margin');
                if($data >  0) { 
                   $view =  '<button type="button" class="btn btn-primary btn-xs btn-modal no-print" id="view_s" data-container=".view_modal" data-href="' . action('ProductController@viewUnrecieved', [$row->product_id]) .'">' . __('lang_v1.view_Details') . '</button>'; 
                }  
                return $data . ' ' . $row->unit  . $view ;
            })
            ->addColumn('total_delivered', function ($row) {
               
                
                // $data =  TransactionSellLine::OrderBy('id','desc')->where('product_id',$row->product_id)->whereHas('transaction',function($query){
                //                         $query->whereIn('type',['sell','sale']);
                //                         $query->whereIn('status',['final','delivered','draft']);
                //                         $query->whereIn('sub_status',['final','f','proforma']);
                //                     })->get()->sum('margin');
                $data =  TransactionSellLine::OrderBy('id','desc')->whereHas('product',function($query) use($row){
                                        $query->where('id',$row->product_id);
                ...
                if($data >  0) { 
                    $view =  '<button type="button" class="btn btn-primary btn-xs btn-modal no-print" id="view_s" data-container=".view_modal" data-href="' . action('ProductController@viewDelivered', [$row->product_id]) .'">' . __('lang_v1.view_Details') . '</button>'; 
                 }      
                return $data . ' ' . $row->unit . $view;
            })
            ->addColumn('reserved', function ($row) {


                 $reserved = Transaction::whereIn('type', ['sale',"sell"])
                ->whereIn('status', ['ApprovedQuotation' ,"draft" ])
                ->whereIn('sub_status', ["proforma"])
                ->select("id")->get();
                ...
                foreach($reserved as $value){
                    $qty = TransactionSellLine::where('transaction_id',$value["id"])
                                                ->where("product_id",$row->product_id)
                                                ->select(DB::raw("SUM(quantity) as quantities"))
                                                ->first();

                    $trans_id_    =  $trans_id_ + $qty->quantities;

                    $del_previous = DeliveredPrevious::where('transaction_id',$value["id"])
                                                       ->where("product_id",$row->product_id)
                                                       ->select(DB::raw("SUM(current_qty) as current","id"))
                                                       ->first();
                    if($del_previous){
                        $total_delivered = $total_delivered + $del_previous->current;
                    }
                }
                                                 

                $trans_id = $trans_id_ - $total_delivered;
                
                    $view = "";
                if($trans_id > 0){
                    $view =  '<button type="button" class="btn btn-primary btn-xs btn-modal no-print" id="view_s" data-container=".view_modal" data-href="' . action('ProductController@viewDeliveredf', [$row->product_id , "f"]) .'">' . __('lang_v1.view_Details') . '</button>'; 
                }                          
                return $trans_id . $row->unit ."<br>". $view;    

            })  
            ->addColumn('price_one_purchase', function ($row) {
                $stock              =  0 ;$cost = 0;
                $item_move          = \App\Models\ItemMove::where("product_id",$row->product_id)->whereIn("state",["purchase","opening_stock"])->orderByRaw('ISNULL(date), date desc, created_at desc')->orderBy("id","desc")->orderBy("order_id","desc")->first();
                if(!empty($item_move)){
                    $stock = $item_move->current_qty;
                    $cost  = $item_move->unit_cost;
                }
                $total = 1 *  $cost;
                $final = '<span class="price_one_purchase display_currency " data-orig-value="' . $total . '" data-currency_symbol=true > ' . number_format($total, 2, '.', '').'<span>';
               
             return  $final;
            })   
            ->editColumn('stock_price', function ($row) {
                // with all qty from this product
            ...
            ->addColumn('stock_price_curr', function ($row) {
                ...
                $stock              =  0 ;$cost = 0;$current = 0;
                $item_move          = \App\Models\ItemMove::where("product_id",$row->product_id)->whereIn("state",["purchase","opening_stock"])->orderByRaw('ISNULL(date), date desc, created_at desc')->orderBy("id","desc")->orderBy("order_id","desc")->first();
                if(!empty($item_move)){
                    $stock   = $item_move->current_qty;
                    $current = $item_move->row_price_inc_exp;
                    $cost    = $item_move->unit_cost;
                }
                $total = $current;
                $final = '<span class="price_one_purchase display_currency " data-orig-value="' . $total . '" data-currency_symbol=true > ' . number_format($total, 2, '.', '').'<span>';
            ...
            ->addColumn('price_one_sell', function ($row) {
                $stock            =  0 ;$prc = 0;
                $item_stock       = \App\Models\ItemMove::where("product_id",$row->product_id)->orderByRaw('ISNULL(date), date desc, created_at desc')->orderBy("id","desc")->orderBy("order_id","desc")->first();
                $item_move        = \App\Models\ItemMove::where("state","sale")->where("product_id",$row->product_id)->orderByRaw('ISNULL(date), date desc, created_at desc')->orderBy("id","desc")->orderBy("order_id","desc")->first();
                if(!empty($item_stock)){
                    $stock = $item_stock->current_qty;
                }
                if(!empty($item_move)){
                    $prc = $item_move->row_price_inc_exp;
                }
                 $stock_price = 1 * $prc;
            ...
            ->editColumn('stock_value_by_sale_price', function ($row) {
                ...
                if(!empty($item_move)){
                 ...
                 $stock_price = round($stock,config("constants.currency_precision")) * round($prc,config("constants.currency_precision"));
                return  '<span class="stock_value_by_sale_price display_currency" data-orig-value="' . (float)$stock_price . '" data-currency_symbol=true > ' . (float)$stock_price . '</span>';
            })
            ...
            ->addColumn('profit_one_price', function ($row) {

                $stock              =  0 ;$cost = 0;
                $item_stock         = \App\Models\ItemMove::where("product_id",$row->product_id)->orderByRaw('ISNULL(date), date desc, created_at desc')->orderBy("id","desc")->orderBy("order_id","desc")->first();
                $item_move_purchase = \App\Models\ItemMove::where("product_id",$row->product_id)->whereIn("state",["purchase","opening_stock"])->orderByRaw('ISNULL(date), date desc, created_at desc')->orderBy("id","desc")->orderBy("order_id","desc")->first();
                if(!empty($item_stock)){
                    $stock = $item_stock->current_qty;
                }
                if(!empty($item_move_purchase)){
                    $cost  = $item_move_purchase->unit_cost;
                }
                $total = 1 *  $cost;

                $prc = 0;
                $item_move_sale   = \App\Models\ItemMove::where("state","sale")->where("product_id",$row->product_id)->orderByRaw('ISNULL(date), date desc, created_at desc')->orderBy("id","desc")->orderBy("order_id","desc")->first();
                if(!empty($item_move_sale)){
                    $prc = $item_move_sale->row_price_inc_exp;
                }
                $stock_price = 1 * $prc;

                $final = $stock_price - $total;
                $final = '<span class="profit_one_price display_currency " data-orig-value="' . (float)$final. '" data-currency_symbol=true > ' . number_format($final, config("constants.currency_precision"), '.', '').'<span>';
            ...
            ->addColumn('potential_profit', function ($row) {
                ...
                $total = round($stock,config("constants.currency_precision")) *  round($cost,config("constants.currency_precision"));
                ...
                $final = $stock_price - $total;
                $final = '<span class="potential_profit display_currency " data-orig-value="' . (float)$final. '" data-currency_symbol=true > ' . number_format($final, config("constants.currency_precision"), '.', '').'<span>';
            ...
            ->editColumn('total_sold', function ($row) {
                $total_sold = 0;
                $total_wrong_sold=0;
                $total_correct_sold=0;
                if ($row->total_delivered) {
                    $total_correct_sold =  (float)$row->total_delivered;
                }
                if ($row->total_wrong_delivered) {
                    $total_wrong_sold =  (float)$row->total_wrong_delivered;
                }
                $total_sold = $total_wrong_sold + $total_correct_sold;
            ...
            ->editColumn('total_transfered', function ($row) {
                ...
                if($row->total_transfered != 0){
                    $BTN = "<br><button type='button'  class='btn btn-primary btn-xs btn-modal no-print' data-container='.view_modal' data-href='".action('ProductController@getTrans',[$row->product_id])."' >View</button>";
                }else{
                    $BTN = "";
                }
                return '<span data-is_quantity="true" class="display_currency total_transfered" data-currency_symbol=false data-orig-value="' . $total_transfered . '" data-unit="' . $row->unit . '" >' . $total_transfered . '</span> ' . $row->unit . $BTN;
            })
            ...
            ->setRowAttr([
                 'href' => function ($row) {
                        return  action('ProductController@viewStockDetails', [$row->product_id]) ;
            ...
            $raw_columns  = ['action', "total_delivered" ,'reserved' , "total_recieve" , 'unit_price','price_one_sell','profit_one_price','price_one_purchase',  'total_transfered', 'total_sold','default_purchase_price','stock_price_curr',
        ...
        return view('report.stock_report')
            ->with(compact('categories','currency_details', 'brands', 'units', 'business_locations', 'show_manufacturing_data'));
    }
        ...
        if ($request->ajax()) {
            ...
            $product_details =  $query->select(
                ...
                DB::raw("(SELECT SUM(IF(transactions.type='Stock_Out', TSL.quantity, 0) ) FROM transactions 
                        LEFT JOIN transaction_sell_lines AS TSL ON transactions.id=TSL.transaction_id
                        WHERE transactions.status='final' AND transactions.type='Stock_Out' $location_filter 
    ...
    public function getTaxDetails(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $sells = Transaction::leftJoin('tax_rates as tr', 'transactions.tax_id', '=', 'tr.id')
                ...
                if ($type == 'sale') {
                    $sells->where('transactions.type', 'sale')
    ...
    public function getTaxReport(Request $request)
    {
        ...
        return view('report.tax_report')
            ->with(compact('business_locations', 'taxes', 'currency_details' ,  'tax_report_tabs'));
    }
    ...
    public function getExpenseReport(Request $request)
    {
        ...
        $labels = [];
         foreach ($expenses as $key => $expense) {
            $values[$key] = (float) $expense->amount;
            $labels[$key] = !empty($expense->note) ? $expense->note : __('report.others');
        }
        ...
        return view('report.expense_report')
                    ->with(compact( 'categories', 'values','business_locations', 'expenses','labels'));
    }
    ...
    public function getSalesRepresentativeReport(Request $request)
    {
        ...
        $users     =  [];
        $uss               = User::where('business_id', $business_id)
                                ->where('status', 'active')
                                ->where('allow_login', "1")
                                ->where('is_cmmsn_agnt', 0)->get();
        foreach($uss as $it){

            $users[$it->id] = $it->first_name;
        }
        return view('report.sales_representative')
                ->with(compact('users','currency_details','agents', 'business_locations'));
    }
   ...
   public function getSalesRepresentativeTotalSell(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $sell_details = $this->transactionUtil->getSellTotals($business_id, $start_date, $end_date, $location_id, $created_by,$reportSetting);
    ...
    {
         if (!auth()->user()->can('contacts_report.view')  && !auth()->user()->can('ReadOnly.views') &&  !auth()->user()->can('warehouse.views')) {
            abort(403, 'Unauthorized action.');
        }
    ...
    public function getproductPurchaseReport(Request $request)
    {
            ...
            return Datatables::of($query)
                ...
                ->editColumn('trans_id', function ($row) {
                 ...
                 ->editColumn('subtotal', function ($row) {
                  ...
                  ->addColumn('unitCost', function ($row) {
                    $unit_cost = \App\Product::product_cost($row->product_id);
                     return '<span class="display_currency row_subtotal" data-currency_symbol=true data-orig-value="' . $unit_cost . '">' . $unit_cost . '</span>';
                 })
                ...
                ->rawColumns(['trans_id','unitCost','ref_no', 'unit_purchase_price', 'subtotal', 'purchase_qty', 'quantity_adjusted', 'supplier'])
                ->make(true);
        }
        ...
        return view('report.product_purchase_report')
            ->with(compact('business_locations','currency_details', 'suppliers'));
    }
    ...
    public function getproductSellReport(Request $request)
    {
        ...
        return view('report.product_sell_report')
            ->with(compact('business_locations','currency_details', 'customers'));
    }
//sell for return
 public function getproductSellReturnReport(Request $request)
    ...
    public function sellPaymentReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $query = TransactionPayment::leftjoin('transactions as t', function ($join) use ($business_id) {
                $join->on('transaction_payments.transaction_id', '=', 't.id')
                    ->where('t.business_id', $business_id)
                    ->whereIn('t.type', ['sale', 'opening_balance']);
            })
                ->leftjoin('contacts as c', 't.contact_id', '=', 'c.id')
                ->leftjoin('customer_groups AS CG', 'c.customer_group_id', '=', 'CG.id')
                ->where('transaction_payments.business_id', $business_id)
                ->where(function ($q) use ($business_id, $contact_filter1, $contact_filter2, $parent_payment_query_part) {
                    $q->whereRaw("(transaction_payments.transaction_id IS NOT NULL AND t.type IN ('sale', 'opening_balance') $parent_payment_query_part $contact_filter1)")
                        ->orWhereRaw("EXISTS(SELECT * FROM transaction_payments as tp JOIN transactions ON tp.transaction_id = transactions.id WHERE transactions.type IN ('sale', 'opening_balance') AND transactions.business_id = $business_id AND tp.parent_id=transaction_payments.id $contact_filter2)");
                })
                ->select(
                    DB::raw("IF(transaction_payments.transaction_id IS NULL, 
                                (SELECT c.name FROM transactions as ts
        ...
        return view('report.sell_payment_report')
            ->with(compact('business_locations', 'currency_details' ,'customers', 'payment_types', 'customer_groups'));
    }
    ...
    public function productStockDetails()
    {
        ...
        if (!empty(request()->input('location_id'))) {
            ...
            $stock_details = $query->select(
                ...
                DB::raw("(SELECT SUM(COALESCE(TSL.quantity,0)) FROM transactions 
                        LEFT JOIN transaction_sell_lines AS TSL ON transactions.id=TSL.transaction_id
                        WHERE transactions.status='final' AND transactions.type='Stock_Out' AND transactions.location_id=$location_id 
                ...
                DB::raw("(SELECT SUM(COALESCE(PL.quantity,0)) FROM transactions 
                        ...
                        WHERE transactions.status='received' AND transactions.type='Stock_In' AND transactions.location_id=$location_id 
    ...
    public function getProfit($by = null)
    {
        ...
        $purchase_id = [] ; 
        $sales_id    = [] ;
       $querys = \App\Transaction::where("type","sale")
                                ->whereNotIn("status",['proforma','draft','ApprovedQuotation'])
                                ->get(); 
        
      $list_of_products     = [];
        foreach($querys as $its){
                $product_list = [];
                $sell = \App\TransactionSellLine::where("transaction_id",$its->id)->get();
                foreach($sell as $it){
                    if(!in_array($it->product_id,$list_of_products)){
                        $list_of_products[] = $it->product_id;
                    }
                    $product_list[]  = $it->product_id ;
                }
                $TransactionSellLine = \App\TransactionSellLine::where("transaction_id",$its->id)->whereIn("product_id",$product_list)->select(DB::raw("SUM(quantity) as total"))->first()->total;
                $DeliveredPrevious   = \App\Models\DeliveredPrevious::where("transaction_id",$its->id)->whereIn("product_id",$product_list)->select(DB::raw("SUM(current_qty) as total"))->first()->total;
                $wrong               = \App\Models\DeliveredWrong::where("transaction_id",$its->id)->select(DB::raw("SUM(current_qty) as total"))->first()->total;
         ...
         $queryp = \App\Transaction::where("type","purchase")
                                ->whereNotIn("status",['ordered','pending'])
                                ->get(); 
       foreach($queryp as $itp){
            $product_list = [];
            $purchase = \App\PurchaseLine::where("transaction_id",$itp->id)->get();
            foreach($purchase as $it){
                $product_list[] = $it->product_id;
            }
            $Purchaseline     = \App\PurchaseLine::where("transaction_id",$itp->id)->whereIn("product_id",$product_list)->select(DB::raw("SUM(quantity) as total"))->first()->total;
            $RecievedPrevious = \App\Models\RecievedPrevious::where("transaction_id",$itp->id)->whereIn("product_id",$product_list)->select(DB::raw("SUM(current_qty) as total"))->first()->total;
            $wrong            =  \App\Models\RecievedWrong::where("transaction_id",$itp->id)->select(DB::raw("SUM(current_qty) as total"))->first()->total;
            if($Purchaseline <= $RecievedPrevious){
                $state   = "received";
                $purchase_id[] = $itp->status ;
            }else{
                if($Purchaseline  != $RecievedPrevious){
                    if($itp->status  == "received"){
                        $state   = "final";
                        $purchase_id[] = $itp->status ;
                    }else{
                        $state   = $itp->status;
        ...
        $query = TransactionSellLine
            ...
            ->where('sale.type', 'sale')
             ->whereNotIn('sale.status', ['proforma','draft','ApprovedQuotation'])
        ...
        if ($by == 'product') {
            $query->join('variations as V', 'transaction_sell_lines.variation_id', '=', 'V.id')
                ...
                ->addSelect('P.id',DB::raw("IF(P.type='variable', CONCAT(P.name, ' - ', PV.name, ' - ', V.name, ' (', V.sub_sku, ')'), CONCAT(P.name, ' (', P.sku, ')')) as product"))
                ->groupBy('V.id');
        }
        ...
        if ($by == 'category') {
            $query->join('variations as V', 'transaction_sell_lines.variation_id', '=', 'V.id')
                ...
                ->addSelect('C.id',"C.name as category")
                ->groupBy('C.id');
        }
        ...
        if ($by == 'brand') {
            $query->join('variations as V', 'transaction_sell_lines.variation_id', '=', 'V.id')
                ...
                ->addSelect('B.id',"B.name as brand")
                ->groupBy('B.id');
        }
        ...
        if (in_array($by, ['invoice'])) {
              $profit=0;
              $datatable->editColumn( 'gross_profit', function($row) use($list_of_products,$profit) {
                $row->transaction_id;
                $tr = \App\TransactionSellLine::where("transaction_id",$row->transaction_id)->get();
                $list_pro  =  [];
                foreach($tr as $i){
                    if(!in_array($i->product_id,$list_pro)){
                         $list_pro[] = $i->product_id; 
                    }
                } 
                $product_separate = [];
                foreach($list_pro as $prod){
                    if(in_array($prod,$list_of_products)){
                         $item_move = \App\Models\ItemMove::where("state","sale")->where("product_id",$prod)->get();
                         foreach($item_move as $it){
                            $qty                = $it->qty;
                            $row_price_inc_exp  = $it->row_price_inc_exp;
                            $out_price          = $it->out_price;
                            $profit            += $qty*($row_price_inc_exp - $out_price);
                            $product_separate[$prod] =  $qty*($row_price_inc_exp - $out_price);
                        }
                    }
                } 
                ...
                $html .= '<hr>';
                foreach($product_separate as $key => $value){
                    $pro = \App\Product::find($key);
                    $html .= '<br>&nbsp;&nbsp;&nbsp;<span>'.$pro->name.' : </span> &nbsp;<span class="display_currency gross-profit" data-currency_symbol="true" data-orig-value="' . $value . '">'.$value.'</span> ';
                }
                return $html;
            });
        } else  if (in_array($by, ['product'])) {
            $profit = 0;
            $datatable->editColumn( 'gross_profit', function($row) use($list_of_products,$profit) {
                if(in_array($row->id,$list_of_products)){
                     $item_move = \App\Models\ItemMove::where("state","sale")->where("product_id",$row->id)->get();
                     foreach($item_move as $it){
                        $qty                = $it->qty;
                        $row_price_inc_exp  = $it->row_price_inc_exp;
                        $out_price          = $it->out_price;
         ...
         } else  if (in_array($by, ['category'])) {
            $profit = 0;
            $datatable->editColumn( 'gross_profit', function($row) use($list_of_products,$profit) {
                 $pro = \App\Product::where("category_id",$row->id)->get();
                 foreach($pro as $it){
                    if(in_array($it->id,$list_of_products)){
                         $item_move = \App\Models\ItemMove::where("state","sale")->where("product_id",$it->id)->get();
                         foreach($item_move as $it){
                            $qty                = $it->qty;
                            $row_price_inc_exp  = $it->row_price_inc_exp;
                            $out_price          = $it->out_price;
        ...
        }  else  if (in_array($by, ['brand'])) {
            $profit = 0;
            $datatable->editColumn( 'gross_profit', function($row) use($list_of_products,$profit) {
                 $pro = \App\Product::where("brand_id",$row->id)->get();
                 foreach($pro as $it){
                    if(in_array($it->id,$list_of_products)){
                         $item_move = \App\Models\ItemMove::where("state","sale")->where("product_id",$it->id)->get();
                         foreach($item_move as $it){
    ...
    public function itemsReport()
    {
        ...
        $customers = Contact::customersDropdown($business_id, false);
        $business_locations = BusinessLocation::forDropdown($business_id);
        return view('report.items_report')->with(compact('suppliers','currency_details', 'customers', 'business_locations'));
    }
...
//dd($business_locations);
    ...
    public function getStockValue()
    {
        ...
        //     $filters
        // );
        $closing_stock_by_sp = 0;  
        $product  = \App\Product::where("business_id",$business_id)->get();
        foreach($product as $it){
            $rod = \App\Variation::where("product_variation_id",$it->id)->first();
            if(!empty($rod)){
            ...
            $closing_stock_by_sp +=  $po * $total;
        }


        $closing = \App\Product::closing_stock($business_id);
        
        $potential_profit = $closing_stock_by_sp - $closing;

        
        if($closing_stock_by_sp != 0){
            $ex = ($potential_profit / $closing_stock_by_sp) * 100;
            $x = ( intval($closing_stock_by_sp) == 0 ) ? 0 : $ex;
            $profit_margin = empty($closing_stock_by_sp) ? 0 : $x;
        }else{
            $ex = ($potential_profit / 1) * 100;
            $x = ( intval($closing_stock_by_sp) == 0 ) ? 0 : $ex;
            $profit_margin = empty($closing_stock_by_sp) ? 0 : $x;
        }
          
        return [
            'closing_stock_by_pp' => $closing,
    ...
    public function activityLog()
    {
        ...
        $transaction_types = [
            ...
            'Stock_Out' => __('lang_v1.stock_transfer'),
        ...
        if (request()->ajax()) {
            ...
            if (!empty($subject_type)) {
                if ($subject_type == 'contact') {
                    ...
                    'sales_order', 'purchase_order', 'sell_return', 'purchase_return', 'Stock_Out', 'expense', 'purchase_order'])) {
