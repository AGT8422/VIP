...
class ContactUtil extends Util
{
    ...
    public function getContactInfo($business_id, $contact_id)
    {
        $contact = Contact::where('contacts.id', $contact_id)
                    ...
                    ->select(
                        ...
                        DB::raw("SUM(IF(t.type = 'sale' AND t.status = 'final', (SELECT SUM(IF(is_return = 1,-1*amount,amount)) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)) as invoice_received"),
    ...
    public function createNewContact($input)
    {
        ...
        if ($count == 0) {
            ...
            $ref_count = $this->setAndGetReferenceCount( $type, $input['business_id']);

            if (empty($input['contact_id'])) {
                //Generate reference number
                $input['contact_id'] = $this->generateReferenceNumber( $type, $ref_count, $input['business_id']);
            }
    ...
    public function updateContact($input, $id, $business_id)
    {
        ...
        if ($count == 0) {
            ...
            if($account != null){
                $account->name           = $input["first_name"];
                $account->account_number = $input["contact_id"];
                $account->save();
            }
    ...
    public function getContactQuery($business_id, $type, $contact_ids = [])
    {
        ...
        if (in_array($type, ['supplier', 'both'])) {
            $query->addSelect([
                DB::raw("SUM(IF(t.type = 'purchase' AND t.due_state = 0 , final_total, 0)) as total_purchase"),
                DB::raw("SUM(IF(t.type = 'purchase', (SELECT SUM(amount) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)) as purchase_paid"),
