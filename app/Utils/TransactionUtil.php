...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $sale_type = !empty($input['type']) ? $input['type'] : 'sale';
        ...
        $ddiscount   = $uf_data ? $this->num_uf($input['discount_amount']) : $input['discount_amount'];
        $ttax = $uf_data ? $this->num_uf($invoice_total['tax']) : $invoice_total['tax'];
       
        $agent_id    =  (isset($input['agent_id']))?$input['agent_id']:null;
        $cost_center_id    =  (isset($input['cost_center_id']))?$input['cost_center_id']:null;
        ...
        $transaction = Transaction::create([
            ...
            'tax_amount' => round($ttax,config('constants.currency_precision')),
            'final_total' => round($final_total,config('constants.currency_precision')),
    ...
    public function updateSellTransaction($transaction_id, $business_id, $input, $invoice_total, $user_id, $uf_data = true, $change_invoice_number = true)
    {
        ...
        if ($transaction->status != $input['status'] && $change_invoice_number) {
            if($input["sub_status"] == "quotation"){
                $invoice_scheme_id = !empty($input['invoice_scheme_id']) ? $input['invoice_scheme_id'] : null;
                $invoice_no = $this->getInvoiceNumber($business_id, $input["sub_status"], $transaction->location_id, $invoice_scheme_id);
            } else if( $input["status"] == "draft" && $input["sub_status"] == null  ) {
                $invoice_scheme_id = !empty($input['invoice_scheme_id']) ? $input['invoice_scheme_id'] : null;
                $invoice_no = $this->getInvoiceNumber($business_id, $input["status"], $transaction->location_id, $invoice_scheme_id);
            } else if($input["sub_status"] == "proforma"  ) {
                $invoice_scheme_id = !empty($input['invoice_scheme_id']) ? $input['invoice_scheme_id'] : null;
                $invoice_no = $this->getInvoiceNumber($business_id, $input["sub_status"], $transaction->location_id, $invoice_scheme_id);
            }  else if( $input["status"] == "final" || $input["status"] == "delivered" )  {
                $invoice_scheme_id = !empty($input['invoice_scheme_id']) ? $input['invoice_scheme_id'] : null;
                $invoice_no = $this->getInvoiceNumber($business_id, $input['status'], $transaction->location_id, $invoice_scheme_id);
            }
        }
        $final_total = $uf_data ? $this->num_uf($input['final_total']) : $input['final_total'];
        $ddiscount   = $uf_data ? $this->num_uf($input['discount_amount']) : $input['discount_amount'];
        $ttax = $uf_data ? $this->num_uf($input['tax_amount']) : $input['tax_amount'];
        $agent_id    =  (isset($input['agent_id']))?$input['agent_id']:null;
        $cost_center_id        =  (isset($input['cost_center_id']))?$input['cost_center_id']:null;
        $update_date = [
            'agent_id' => $agent_id,
            ...
            'discount_amount' => round($ddiscount,config('constants.currency_precision')),
            'tax_amount' => round($ttax,config('constants.currency_precision')),
            'final_total' => round($final_total,config('constants.currency_precision')),
            ...
            ,'pattern_id' => (!empty($input['pattern_id'])) ? $input['pattern_id'] : null
            ,'currency_id' => (!empty($input['currency_id'])) ? $input['currency_id'] : null
            ,'exchange_price' => (!empty($input['currency_id'])) ? $input['currency_id_amount'] : null 
            ,'amount_in_currency' => (!empty($input['currency_id'])) ? (round($final_total,2) / $input['currency_id_amount']) : null 
        ];
     ...
     */
    public function createOrUpdateSellLines($transaction, $products, $location_id, $return_deleted = false, $status_before = null, $extra_line_parameters = [], $uf_data = true,$tax_rate=null,$archive=null,$request=null)
    {
        ...
        foreach ($products as $key => $product) {
            ...
            if (!empty($product['transaction_sell_lines_id'])) {
                ...
                $edit_id_temp = $this->editSellLine($product, $location_id, $status_before, $multiplier,$tax_rate,$request,$key);
                ...
                $variation   = \App\Variation::where("product_id",$product['product_id'])->first();
                $line = [
                    'se_note' => (isset($product['se_note']))?$product['se_note']:null,
                    ...
                    'line_discount_amount' => !empty($product['line_discount_amount___']) ? $uf_data ? $this->num_uf($product['line_discount_amount___']) : $product['line_discount_amount___'] : 0,
                    'item_tax' =>  $uf_item_tax / $multiplier,
                    'tax_id' => $product['tax_id'],
                    'order_id' => ( $request != null && isset($request->line_sort))?$request->line_sort[$key]:null,
        ...
        if (!empty($edit_ids)) {
            ...
            $ids = [];
            foreach($deleted_lines as $it){
                $ids[] = $it["id"];
            }
            $combo_delete_lines = TransactionSellLine::whereIn('parent_sell_line_id',$ids)->select('id')->get()->toArray();
            $deleted_lines = array_merge($deleted_lines, $combo_delete_lines);
            $adjust_qty = $status_before == 'draft' ? false : true;
            $this->deleteSellLines($ids, $location_id, $adjust_qty);
        }
        ...
        if($archive != null){
            $sell_lines = \App\TransactionSellLine::where("transaction_id",$transaction->id)->get();
    ...
    /**
     * Edit transaction sell line **no update value in stock 
     *
     * @param array $product
     ...
     * @return boolean
     */
    public function editSellLine($product, $location_id, $status_before, $multiplier = 1,$tax_rate,$request=null,$key=null)
    {
        ...
        $unit_price = $unit_price_before_discount;
        if (!empty($product['line_discount_type']) && $product['line_discount_amount___']) {
            $discount_amount = $this->num_uf($product['line_discount_amount___']);
            if ($product['line_discount_type'] == 'fixed') {
                $unit_price = $unit_price_before_discount - $discount_amount;
            } elseif ($product['line_discount_type'] == 'percentage') {
                $unit_price = $unit_price_before_discount - $discount_amount;
        ...
        $tax = \App\TaxRate::where("id",$tax_rate)->first();
        if(!empty($tax)){
            $taxx = $tax->amount;
            $final_tax = ($unit_price*$taxx)/100;
        }else{
            $final_tax = $product['tax_id'];
        }
         //Update sell lines.
        ...
        'variation_id' => $product['variation_id'],
            ...
            'se_note' => (isset($product['se_note']))?$product['se_note']:null,
            'line_discount_type' => !empty($product['line_discount_type']) ? $product['line_discount_type'] : null,
            'line_discount_amount' => !empty($product['line_discount_amount___']) ? $this->num_uf($product['line_discount_amount___']) : 0,
            'item_tax' => $this->num_uf($final_tax) / $multiplier,
            'tax_id' => $product['tax_id'],
            'order_id' => ($request != null && isset($request->line_sort))?$request->line_sort[$key]:null,
        ...
        foreach ($payments as $payment) {
            ...
            if (!empty($payment['payment_id'])) {
                ...
                if ($payment_amount > -1) {
                    ...
                    $DATE_SELL = $payment['paid_on'];
                    if (!empty($payment['paid_on'])) {
                        $paid_on = $uf_data ? $this->uf_date($DATE_SELL, true) : $DATE_SELL;
                    } else {
             ...
             if ($payment['amount']) {
                if ($payment['cheque_number']) {
                    \App\Models\Check::add_check($transaction,$payment);
                }elseif($payment['method'] != 'cheque' && $payment['method'] != 'other'){
                    if($transaction->type == 'purchase'){
                       \App\AccountTransaction::add_suplier($transaction,$payment); 
                    }else{
                      \App\AccountTransaction::add_customer($transaction,$payment);  
                    }
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        $currency = \App\Currency::find($transaction->business->currency_id); 
        ...
        $output['useful_id'] = ($transaction->business)?$transaction->business->currency_id:null  ;
        ...
        $output['project_no'] = $transaction->project_no;
       ...
       /* dd( $output['invoice_heading']);*/
        ...
        $output['lines'] = [];
        $total_exempt = 0;
        if (in_array($transaction_type, ['sale', 'sales_order'])) {
        ...
        $output['subtotal'] = ($transaction->total_before_tax != 0) ? (number_format($transaction->total_before_tax,3) ." ". $currency_symbol):number_format(0,3) ." ". $currency_symbol;
        ...
        $output['taxed_subtotal'] = number_format($taxed_subtotal,3)." ". $currency_symbol ;

        //Discount
        $discount_amount = number_format($transaction->discount_amount,3)." ". $currency_symbol;
        ...
        $output['discount'] = ($discount != 0) ? (number_format($discount,3) ." ". $currency_symbol)  : 0;
        $output['total_discount']=number_format(($total_line_discount+$discount),3);

        //reward points
        if ($business_details->enable_rp == 1 && !empty($transaction->rp_redeemed)) {
            $output['reward_point_label'] = $business_details->rp_name;
            $output['reward_point_amount'] = number_format($transaction->rp_redeemed_amount,3)." ". $currency_symbol;
        }

        //Format tax
        if (!empty($output['taxes'])) {
            foreach ($output['taxes'] as $key => $value) {
                $output['taxes'][$key] = number_format($value,3);
            }
        }
        ...
        $output['tax_label'] .= ':';
        $output['tax'] = ($transaction->tax_amount != 0) ? number_format($transaction->tax_amount,3)." ". $currency_symbol : 0;
        ...
        if ($transaction->tax_amount != 0 && $tax->is_tax_group) {
            ...
            foreach ($transaction_group_tax_details as $value) {
                $output['group_tax_details'][$value['name']] = number_format($value['calculated_tax'],3)." ". $currency_symbol;
            }
        }

        //Shipping charges
        $output['shipping_charges'] = ($transaction->shipping_charges != 0) ? number_format( $transaction->shipping_charges,3)." ". $currency_symbol : 0;
        ...
        $output['packing_charge'] = ($transaction->packing_charge != 0) ? number_format($transaction->packing_charge,3)." ". $currency_symbol : 0;

        //Total
        if ($transaction_type == 'sell_return') {
            $output['total_label'] = $invoice_layout->cn_amount_label . ':';
            $output['total'] = number_format($transaction->final_total,3)." ". $currency_symbol;
        } else {
        ...
        if ($transaction_type == 'sale' && $transaction->status == 'final') {
            $paid_amount = $this->getTotalPaid($transaction->id);
            $due         = $transaction->final_total - $paid_amount;

            $output['total_paid']       = ($paid_amount == 0) ? 0 :  number_format($paid_amount,3)." ". $currency_symbol;
            $output['total_paid_label'] = $il->paid_label;
            $output['total_due']        = ($due == 0) ? 0 :  number_format($due, 3)." ". $currency_symbol;
            ...
            if ($il->show_previous_bal == 1) {
                ...
                if (!empty($all_due)) {
                    ...
                    $output['all_due']       = number_format($all_due,3)." ". $currency_symbol;
                }
            }
    ...
    protected function _receiptDetailsSellLines($lines, $il, $business_details)
    {
        ...
        foreach ($lines as $line) {
            ...
            $line_array = [
                ...
                'unit_price_inc_tax' => number_format($line->unit_price_inc_tax,3) ,
                'unit_price_inc_tax_uf' => number_format($line->unit_price_inc_tax,3),
                'unit_price_exc_tax' => number_format($line->unit_price,3) ,
                'price_exc_tax' => number_format($line->quantity * $line->unit_price,3),
                'unit_price_before_discount' => number_format($line->unit_price_before_discount,3) ,
                'unit_price_before_discount_uf' => number_format($line->unit_price_before_discount,3),
                //Fields for 4th column
                'line_total' => number_format($line->unit_price_inc_tax * $line->quantity,3),
                'line_total_uf' => number_format($line->unit_price_inc_tax * $line->quantity,3),
                'line_total_exc_tax' => number_format($line->unit_price * $line->quantity,3),
                'line_total_exc_tax_uf' => number_format($line->unit_price * $line->quantity,3)
            ];
    ...
    public function getInvoiceNumber($business_id, $status, $location_id, $invoice_scheme_id = null, $sale_type = null)
    {
        if ($status == 'final') {
            ...
            $ref_count = $this->setAndGetReferenceCount('Quantity', $business_id);
            $invoice_no = $this->generateReferenceNumber('Quantity', $ref_count, $business_id);
            return $invoice_no;
        } else if ($status == 'proforma') {
            $ref_count = $this->setAndGetReferenceCount('Approve', $business_id);
            $invoice_no = $this->generateReferenceNumber('Approve', $ref_count, $business_id);
            return $invoice_no;
    ...
    public function getPurchaseTotals($business_id, $start_date = null, $end_date = null, $location_id = null, $user_id = null,$reportSetting=null,$loc=null)
    {
        ...
        $query = Transaction::where('business_id', $business_id)
                        ...
                        ->select(
                            ...
                            DB::raw("SUM((SELECT COALESCE(SUM(tp.amount), 0) FROM transaction_payments as tp WHERE tp.return_payment=0 AND tp.transaction_id=transactions.id)) as total_paid"),
        ...
        if($type!=null){
            ...
            $output['total_purchase_inc_tax']   = ($purchases["total"])?$purchases["total"]->total_amounts:0;
        } 
        $output['total_purchase_exc_tax'] = $purchase_details->total_exc_tax ;
        $output['total_purchase_exc_tax'] = $purchase_details->total_before_tax_sum;
        $output['total_purchase_tax']     = $purchase_details->final_total_sum - $purchase_details->total_exc_tax ;
        // $output['total_purchase_exc_tax'] = ($purchases["total"])?$purchases["total"]->total_amounts:0;
    ...
    public function getSellTotals($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null,$reportSetting=null,$type=null)
    {
        ...
        $query = Transaction::where('transactions.business_id', $business_id)
                    ...
                    ->whereIn('transactions.sub_status', ['final','f'])
                    ->select(
                        DB::raw('SUM(final_total) as total_sell'),
                        DB::raw("SUM(final_total - tax_amount) as total_exc_tax"),
                        DB::raw('SUM(final_total - (SELECT COALESCE(SUM(IF(tp.is_return = 1, -1*tp.amount, tp.amount)), 0) FROM transaction_payments as tp WHERE tp.return_payment=0 AND tp.transaction_id = transactions.id) )  as total_due'),
        ...
        // }else{
            // }
        // $output['total_sell_exc_tax']     = ($sales["total"])?$sales["total"]->total_amounts:0;
        $output['total_sell_exc_tax']     = $sell_details->total_exc_tax;
        $output['total_sell_tax']         = $sell_details->total_sell - $sell_details->total_exc_tax;
        $output['total_sell_inc_tax']     = $sell_details->total_sell  ;
        $output['invoice_due']            = $sell_details->total_due;
        $output['total_shipping_charges'] =  ($sales_shipping["total"])?$sales_shipping["total"]->total_amounts:0;
        $output['accounts'] = $sales["accounts"];
    ...
    public function getOutputTax($business_id, $start_date = null, $end_date = null, $location_id = null)
    {
        ...
        $query1 = Transaction::where('transactions.business_id', $business_id)
                        ...
                        ->whereIn('transactions.sub_status', [ 'final','f'])
                        ->select(
                            DB::raw("SUM( IF(type='sale', transactions.tax_amount, -1 * transactions.tax_amount) ) as transaction_tax"),
                            'T.name as tax_name',
                            'T.id as tax_id',
                            'T.is_tax_group'
                        );
    ...
    */
    ...
    public function getExpenseReport(
        ...
        $out = $this->getTotalByType("expense",$reportSetting,null,null);
         
        $accounts_id = [];
        $accounts_amount = [];
        foreach( $out["accounts"] as $key => $iit ){
                $accounts_id[] = $key;
                $accounts_amount[$key] = $iit;
        }
        
        $query = \App\AccountTransaction::whereIn('account_id', $accounts_id)->select("*") ;
        ...
        if (!empty($filters['start_date']) && !empty($filters['end_date'])) {
            $query->whereBetween('operation_date', [$filters['start_date'],
                $filters['end_date']]);
        }
    ...
    {
        $total_paid = TransactionPayment::where('transaction_id', $transaction_id)->where('return_payment',"=", 0)
                ->select(DB::raw('SUM(IF( is_return = 0, amount, amount*-1)) as total_paid'))
                ->first()
                ->total_paid;
        $check_refund = TransactionPayment::where('transaction_id', $transaction_id)->where('return_payment',"=", 0)->whereHas(
    ...
    public function calculatePaymentStatus($transaction_id, $final_amount = null)
    {
        ...
        $transaction_related_payment = \App\Transaction::where("separate_parent",$transaction_id)->whereNull("sub_type")->where("separate_type","payment")->get();
        $sum_of_payment = 0;
        if(count($transaction_related_payment)>0){
            foreach($transaction_related_payment as $k => $invoice){
                $sum_of_payment += $invoice->final_total;
            }
            $total_paid += $sum_of_payment;
        } 
    ...
    public function payAtOnce($parent_payment, $type)
    {
        ...
        if ($due_transactions->count()) {
            foreach ($due_transactions as $transaction) {
                ...
                if ($transaction->type == 'sale' && $transaction->status != 'final') {
                    continue;
                }
                if ($total_amount > 0) {
    ...
    public function mapPurchaseSell($business, $transaction_lines, $mapping_type = 'purchase', $check_expiry = true, $purchase_line_id = null)
    {
        ...
        foreach ($transaction_lines as $line) {
            ...
            // ->where('transactions.status', 'recieved')
            ...
            $query = Transaction::join('purchase_lines AS PL', 'transactions.id', '=', 'PL.transaction_id')
                ...
                ->whereIn('transactions.type', ['purchase', 'Stock_In',
     ...
     * @return float
     */
    public function getOpeningClosingStock($business_id, $date, $location_id, $is_opening = false, $by_sale_price = false, $filters = [],$loc=null)
    {
        // $transaction_used = \App\Transaction::where("type","sale") 
        ...
        ->whereNotIn('purchase.status', ['ordered','pending'])
    ...
    public function isCustomerCreditLimitExeeded(
        ...
        $credit_details =  $query->select(
            DB::raw("SUM(IF(t.type = 'sale', final_total, 0)) as total_invoice"),
            DB::raw("SUM(IF(t.type = 'sale', (SELECT SUM(IF(is_return = 1,-1*amount,amount)) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)) as invoice_paid")
        )->first();
    ...
    */
    public function recalculateSellLineTotals($business_id, $sell_line)
    {
        ...
        if (!empty($sub_unit)) {
            ...
            $sell_line->quantity                   = number_format($sell_line->quantity / $multiplier,3);
            $sell_line->unit_price_before_discount = number_format($sell_line->unit_price_before_discount * $multiplier,3);
            $sell_line->unit_price                 = number_format($sell_line->unit_price * $multiplier,3);
            $sell_line->unit_price_inc_tax         = number_format($sell_line->unit_price_inc_tax * $multiplier,3);
            $sell_line->item_tax                   = number_format($sell_line->item_tax * $multiplier,3);
            $sell_line->quantity_returned          = number_format($sell_line->quantity_returned / $multiplier,3);
    ...
    public function getTransactionTotals(
        ...
        $sal_discount = $this->getTotalByType("sell_discount",$reportSetting,$start_date,$end_date);
        ...
        if (in_array('Stock_Out', $transaction_types)) {
            $query->addSelect(
                DB::raw("SUM(IF(transactions.type='Stock_Out', shipping_charges, 0)) as total_transfer_shipping_charges")
        ...
        if (in_array('sale', $transaction_types)) {
            $query->addSelect(
                DB::raw("SUM(IF(transactions.type='sale' AND transactions.status='final', IF(discount_type = 'percentage', COALESCE(discount_amount, 0)*total_before_tax/100, COALESCE(discount_amount, 0)), 0)) as total_sell_discount"),
                DB::raw("SUM(IF(transactions.type='sale' AND transactions.status='final', rp_redeemed_amount, 0)) as total_reward_amount"),
                DB::raw("SUM(IF(transactions.type='sale' AND transactions.status='final', round_off_amount, 0)) as total_sell_round_off")
            );
        }
        ...
        if (in_array('purchase_return', $transaction_types)) {
            ...
            // $purchase_return = $this->getTotalByType("purchase_return",$reportSetting,$start_date,$end_date);
            // $output['total_purchase_return_exc_tax'] = ($purchase_return["total"])?($purchase_return["total"]->total_amounts):0; 
        }
        ...
        if (in_array('sell_return', $transaction_types)) {
            ...
            // $sell_return = $this->getTotalByType("sell_return",$reportSetting,$start_date,$end_date);
            // $output['total_sell_return_exc_tax'] = ($sell_return["total"])?($sell_return["total"]->total_amounts):0;  
        }
        ...
        if (in_array('expense', $transaction_types)) {
            ...
            $expenses = $this->getTotalByType("expense",$reportSetting,$start_date,$end_date);
            $output['total_expense'] = $total_expense - $total_expense_refund;
            $output['total_expense'] = ($expenses["total"])?($expenses["total"]->total_amounts):0;
            $output['expenses_accounts'] = ($expenses["total"])?($expenses["accounts"]):null;
        }
        ...
        $reveneus = $this->getTotalByType("reveneus",$reportSetting,$start_date,$end_date);
        $output['total_reveneus'] = ($reveneus["total"])?($reveneus["total"]->total_amounts):0;
        $output['reveneus_accounts'] = ($reveneus["total"])?($reveneus["accounts"]):null;
        ...
        if (in_array('purchase', $transaction_types)) {
            ...
            $purchase_discount = $this->getTotalByType("purchase_discount",$reportSetting,$start_date,$end_date);
            $output['total_purchase_discount'] = ($purchase_discount["total"])?($purchase_discount["total"]->total_amounts):0;  
        }


        if (in_array('sale', $transaction_types)) {
            $sell_discount = $this->getTotalByType("sell_discount",$reportSetting,$start_date,$end_date);
            // $output['total_sell_discount'] =
            //     !empty($transaction_totals->total_sell_discount) ?
            //     $transaction_totals->total_sell_discount : 0;
            $output['total_sell_discount'] = ($sell_discount["total"])?($sell_discount["total"]->total_amounts):0;
    ...
    //...... get TOTAL by Type
    public function getTotalByType($type,$reportSetting,$start_date,$end_date){
        $ids         = [];
        $accounts    = [];
        $total       = [];
            // if($type == "sell_shipping"){
        ...
        if($reportSetting){
            if($type == "sell"){
                $output = $reportSetting->sale;
            }elseif($type   == "purchase"){
                $output = $reportSetting->purchase;
            }elseif($type == "opening"){
                $output = $reportSetting->purchase_opening_stock;
            }elseif($type == "sell_discount"){
                $output = $reportSetting->sale_discount;
            }elseif($type == "sell_return"){
                $output = $reportSetting->sale_return;
            }elseif($type == "expense"){
                $output = $reportSetting->expense;
            }elseif($type == "reveneus"){
                $output = $reportSetting->revenue;
            }elseif($type == "sell_shipping"){
                $output = $reportSetting->sale_addtional_cost;
            }elseif($type == "purchase_shipping"){
                $output = $reportSetting->purchase_addtional_cost;
            }elseif($type == "purchase_discount"){
                $output = $reportSetting->purchase_discount;
            }elseif($type == "purchase_return"){
                $output = $reportSetting->purchase_return;
            }else{
                $output = null;
            }
            ...
            if( $output != null){
                ...
                if(!(count($account)>0)   ){
                    ...
                    $account  = \App\Account::whereHas("account_type",function($query) use($output){
                                                $query->where("sub_parent_id",$output);
                                        })->get();
                ...
                if(!(count($account)>0)  ){
                    ...
                    $account  = \App\Account::whereHas("account_type",function($query) use($output){
                                                    $query->where("accounts.account_type_id",$output);
                                            })->get();
                    
                } 
                foreach($account as $it){
                    if($type == "sell"){
                        if(!in_array($it->account_type_id , [($reportSetting)?$reportSetting->sale_discount:null])){
                            $accounts[]     = $it->id  ;
                            // dd($start_dat    e . " _ " . $end_date);
                            if($start_date != null && $end_date != null){
                                $amount         = \App\AccountTransaction::where("account_id",$it->id)->select(
                                    \DB::raw("SUM(IF(type = 'credit',amount,-1*amount)) as total_amounts")
                        ...
                        if(!in_array($it->account_type_id ,[($reportSetting)?$reportSetting->purchase_discount:null])){
                            $accounts[]     = $it->id  ;
                            // dd($start_dat    e . " _ " . $end_date);
                            if($start_date != null && $end_date != null){
                                $amount         = \App\AccountTransaction::where("account_id",$it->id)->select(
        ...
        if($start_date != null && $end_date != null  && count($accounts) > 0){
            if($type == "opening"){
    ...
    public function getGrossProfit($business_id, $start_date = null, $end_date = null, $location_id = null, $user_id = null,$reportSetting=null,$loc=null)
    {
        ...
        $transaction_totals = $this->getTransactionTotals($business_id, ['sale'], $start_date, $end_date, $location_id, $user_id,$reportSetting,1);
    ...
    public function updateCustomerRewardPoints(
        ...
        if ($customer->is_default == 1) { return false;  }
    ...
    {
        if (session()->has('business')) { $business = session()->get('business'); } else { $business = Business::find($business_id);}
        ...
        if (empty($customer_reward_points) || $customer->is_default == 1) { return $details; }
        $min_reward_point_required = $business->min_redeem_point;
        if (!empty($min_reward_point_required) && $customer_reward_points < $min_reward_point_required) { return $details; }
        $max_redeem_point          = $business->max_redeem_point;
        if (!empty($max_redeem_point) && $max_redeem_point <= $customer_reward_points) { $customer_reward_points = $max_redeem_point; }
    ...
    public function getListPurchases($business_id)
    {
        $purchases = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
                    ...
                    ->select(
                        ...
                        DB::raw('SUM(IF(TP.deleted_at IS NULL, TP.amount, 0)) as amount_paid'),
    ...
    * @return object
    */
    public function getListSells($business_id, $sale_type = 'sale',$styl = null)
    {
        ...
        if ($sale_type == 'sale') {
            if($styl != null){
                 $sells->whereIn('transactions.status', ['final','Delivered',"draft","ApprovedQuotation"]);
            }else{
                $sells->whereIn('transactions.status', ['final','Delivered']);
    ...
    public function getLedgerDetails($contact_id, $start, $end)
    {
        ...
        $previous_transaction_sums = $this->__transactionQuery($contact_id, $start)
                ->select(
                    ...
                    DB::raw("SUM(IF(type = 'sale' AND status = 'final', final_total, 0)) as total_invoice"),
        ...
        foreach ($transactions as $transaction) {
            ...
            $date_i = Carbon::parse($transaction->transaction_date);
            $ledger[] = [
                'id' => $transaction->id,
                'date' => $date_i->format("Y-m-d"),
                'ref_no' => in_array($transaction->type, ['sale', 'sell_return']) ? $transaction->invoice_no : $transaction->ref_no,
                'type' => $transaction_types[$transaction->type],
                'location' => $transaction->location->name,
                'payment_status' =>  __('lang_v1.' . $transaction->payment_status),
                'total' => '',
                'payment_method' => '',
                'debit' => in_array($transaction->type, ['sale', 'purchase_return'])  ? $transaction->final_total : '',
        ...
        foreach ($payments as $payment) {
            ...
            $ref_no = in_array($payment->transaction_type, ['sale', 'sell_return']) ?  $payment->invoice_no :  $payment->ref_no;
            $note = $payment->note;
            if (!empty($ref_no)) {
            ...
            if($payment->check_id != null){
                $ref_  = $payment->check->ref_no;
                $type_ = "Cheque" ;
            }else{
                $ref_  =  $payment->payment_ref_no;
                $type_ =  $transaction_types['payment'];
            }
            $ledger[] = [
                'id' => $payment->id,
                ...
                'credit' => (in_array($payment->transaction_type, ['sale', 'purchase_return', 'opening_balance']) || $payment->is_advance == 1) && $payment->is_return == 0 ? $payment->amount : '',
                'others' =>  $note ,
                'voucher_id' => $payment->payment_voucher_id,
        ...
        //................................................................................................
        //Get cheque totals between dates
        $cheques = $this->__CheuqeQuery($contact_id, $start, $end,$check_ids)
                         ->select([
                            'c.*' ,
                            'checks.id as ck_id',
                            'checks.collect_account_id as ck_collect_account_id',
                            'checks.transaction_id as transaction_id',
                            'checks.ref_no as ck_ref_no',
                            'checks.contact_id as ck_contact_id',
                            'checks.location_id as location_id',
                            'checks.account_id as ck_account',
                            'checks.contact_bank_id as ck_contact_bank_id',
                            'checks.type   as ck_type',
                            'checks.status as ck_status',
                            'checks.amount as ck_amount',
                            'checks.write_date as ck_write_date',
                            'checks.due_date   as ck_due_date',
                            'checks.cheque_no  as ck_cheque_no',
                            'checks.collecting_date as ck_collecting_date',
                            'checks.transaction_payment_id as ck_transaction_payment_id',
                            'checks.note as ck_note',
                            'checks.created_at as ck_created_at',
                            'checks.updated_at as ck_updated_at',
                        ])
                        ->get();  
        foreach ($cheques as $cheque) {
            if($cheque->transaction != null){
                if($cheque->transaction->type == 'opening_balance'){
                    $opening_balance_paid += $cheque->ck_amount;
                }
            }
       
            $ref_no =  $cheque->ck_ref_no;
            $transa_ref_no =  $cheque->transaction;
            $note = $cheque->ck_note;
            if (!empty($transa_ref_no)) {
                $note .='<small>' . __('account.payment_for') . ': ' . $transa_ref_no->ref_no . '</small>';
            }
            
             $ledger[] = [
                'id'       => $cheque->ck_id,
                'date'     => $cheque->ck_write_date ,
                'ref_no'   => $cheque->ck_ref_no,
                'type'     => "Cheque",
                ...
                'debit'      =>  ($cheque->ck_type == 1) ? $cheque->ck_amount : '',
                'credit'     =>  ($cheque->ck_type == 0) ? $cheque->ck_amount : '',
                'others'     =>  strip_tags($note) ,
                'voucher_id' => null,
                'check_id'   => null,
            ];
        }
        //Get cheque totals between dates
        $cheques2 = $this->__CheuqeQuery2($contact_id, $start, $end,$check_ids)
                         ->select([
                            'c.*' ,
                            'checks.id as ck_id',
                            'checks.collect_account_id as ck_collect_account_id',
        ...
        //Get voucher totals between dates
        $vouchers = $this->__VoucherQuery($contact_id, $start, $end)
                        ->select([
                            'c.*' ,
                            'payment_vouchers.id as v_id',
                            'payment_vouchers.business_id as business_id',
                            'payment_vouchers.amount as v_amount',
                            'payment_vouchers.type as v_type',
                            'payment_vouchers.ref_no as v_ref_no',
                            'payment_vouchers.contact_id as v_contact_id',
                            'payment_vouchers.account_id as v_account',
                            'payment_vouchers.text as v_text',
                            'payment_vouchers.date as v_date',
                            'payment_vouchers.created_at as v_created_at',
                            'payment_vouchers.updated_at as v_updated_at',
                        ])
                        ->get();
        //Get voucher totals between dates
        $vouchers2 = $this->__VoucherQuery2($contact_id, $start, $end  )
                        ->select([
                            'c.*' ,
                            'payment_vouchers.id as v_id',
                            'payment_vouchers.business_id as business_id',
        ...
        foreach ($vouchers as $voucher) {
            ...
            $ledger[] = [
                ...
                'ref_no' => $voucher->v_ref_no,
                'type' => "Voucher",
                'location' => $voucher->business->locations->first()->name,
                'payment_status' => '',
                'total'  => "",
                'payment_method' => "Add Voucher",
                'debit'  =>  ($voucher->v_type == 0) ? $voucher->v_amount : '',
                'credit' =>  ($voucher->v_type == 1) ? $voucher->v_amount : '',
                'others' =>  strip_tags($note) ,
                'voucher_id' => null,
                'check_id'   => null,
            ];
        ...
        //Get Journal-Voucher-debit totals between dates
        $vouchers_debit = $this->__JournalVoucherQuery($contact_id, $start, $end)
                                 ->select([
                                    'c.*' ,
                                    'daily_payment_items.id as dp_id',
                                    'daily_payment_items.daily_payment_id as daily_payment_id',
                                    'daily_payment_items.credit as dp_credit',
                                    'daily_payment_items.account_id as dp_account_id',
                                    'daily_payment_items.debit as dp_debit',
                                    'daily_payment_items.text  as dp_text',
                                    'daily_payment_items.created_at as dp_created_at',
                                    'daily_payment_items.updated_at as dp_updated_at',
                                ])
                                ->get();
        foreach ($vouchers_debit as $voucher_debit) {
                                    
                                   
            $ref_no   =  $voucher_debit->daily_payment->ref_no;
            $note     =  $voucher_debit->dp_text ;
            $ledger[] = [
                'id'           => $voucher_debit->dp_id ,
                'date'           => $voucher_debit->daily_payment->date ,
                'ref_no'         => $voucher_debit->daily_payment->ref_no,
                'type'           => "Journal-Voucher",
                'location'       => $voucher_debit->daily_payment->business->locations->first()->name,
                'payment_status' => '',
                'total'          => "",
                'payment_method' => "Add Journal Voucher",
                'debit'          =>  ($voucher_debit->dp_debit == 0)? "":$voucher_debit->dp_debit,
                'credit'         =>  ($voucher_debit->dp_credit == 0)? "":$voucher_debit->dp_credit ,
                'others'         =>  strip_tags($note) ,
                'voucher_id' => null,
                'check_id'   => null,
            ];
        } 
        //Get Expenses totals between dates
        $expenses = $this->__ExpensesQuery($contact_id, $start, $end)
                        ->select([
                            'c.*' ,
                            'additional_shipping_items.id as adi_id',
                            'additional_shipping_items.additional_shipping_id as additional_shipping_id',
                            'additional_shipping_items.contact_id as adi_contact_id',
                            'additional_shipping_items.account_id as adi_account_id',
                            'additional_shipping_items.amount as adi_amount',
                            'additional_shipping_items.vat as adi_vat',
                            'additional_shipping_items.total as adi_total',
                            'additional_shipping_items.text as adi_text',
                            'additional_shipping_items.date as adi_date',
                            'additional_shipping_items.cost_center_id as adi_cost_center_id',
                            'additional_shipping_items.created_at as adi_created_at',
                            'additional_shipping_items.updated_at as adi_updated_at',
                        ])
                        ->get();
        foreach ($expenses as $expense) {
            if($expense->additional_shipping->transaction->status == "received" || $expense->additional_shipping->transaction->status == "final"){
                
                $ref_no   =  $expense->additional_shipping->transaction->ref_no;
                $note     =  $expense->adi_text ;
                $ledger[] = [
                    'id'             => $expense->adi_id ,
                    'date'           => $expense->adi_date ,
                    'ref_no'         => $expense->additional_shipping->transaction->ref_no,
                    'type'           => "Expenses",
                    'location'       => $expense->additional_shipping->transaction->business->locations->first()->name,
                    'payment_status' => '',
                    'total'          => "",
                    'payment_method' => "Add Expense",
                    'debit'          => '',
                    'credit'         => $expense->adi_total  ,
                    'others'         => strip_tags($note) ,
                    'voucher_id' => null,
                    'check_id'   => null,
                ];
        ...
        $total_invoice_paid = $payments->where('transaction_type', 'sale')->where('is_return', 0)->sum('amount') + $cheques->where('type', 'customer')->sum('amount');
        $total_sell_change_return = $payments->where('transaction_type', 'sale')->where('is_return', 1)->sum('amount');
        ...
        $total_purchase_paid = $payments->where('transaction_type', 'purchase')->where('is_return', 0)->sum('amount') + $cheques->where('type', 'supplier')->sum('amount');
    ...
    //........................................................................ ............ .................
    /**
     * Query to get Cheque details for a customer || supplier
     *
     */
    private function __CheuqeQuery($contact_id, $start, $end = null,$check_ids)
    {
        $business_id = request()->session()->get('user.business_id');

        $query = Check::leftJoin(
                    'contacts as c',
                    'checks.contact_id',
                    '=',
                    'c.id'
                )->where('c.id', $contact_id)->whereNotIn("checks.id",$check_ids)->where('checks.account_type', 0);


        if (!empty($start)  && !empty($end)) {
            $query->whereDate('checks.created_at', '>=', $start)
                        ->whereDate('checks.created_at', '<=', $end);
        }
     ...
     *
     */
    private function __CheuqeQuery2($contact_id, $start, $end = null,$check_ids)
    {
        $business_id = request()->session()->get('user.business_id');
        ...
        $query = Check::leftJoin(
                    ...
                    '=',
                    'c.id'
                )->where('c.contact_id', $contact_id)->whereNotIn("checks.id",$check_ids)->where('checks.account_type', 1);
    ...
    /**
     * Query to get JournalVoucherD details for a customer || supplier
     *
     */
    private function __JournalVoucherQuery($contact_id, $start, $end = null)
    {
        $business_id = request()->session()->get('user.business_id');
        $account_id  = \App\Account::where("contact_id",$contact_id)->first();
        $query       = DailyPaymentItem::leftJoin(
                    'accounts as a',
                    'daily_payment_items.account_id',
                    '=',
                    'a.id'
                )->leftJoin(
                    'contacts as c',
                    ...
                    'daily_payments as dp',
                    'daily_payment_items.daily_payment_id',
                    '=',
                    'dp.id'
                )->where('a.id', $account_id->id);


        if (!empty($start)  && !empty($end)) {
            $query->whereDate('dp.date', '>=', $start)
                        ->whereDate('dp.date', '<=', $end);
        }
    ...
    /**
     * Query to get Expenses details for a customer || supplier
     *
     */
    private function __ExpensesQuery($contact_id, $start, $end = null)
    {
     
        $business_id = request()->session()->get('user.business_id');

        $query = AdditionalShippingItem::leftJoin(
                    'contacts as c',
                    'additional_shipping_items.contact_id',
                    '=',
                    'c.id'
        ...
        if (!empty($start)  && !empty($end)) {
            $query->whereDate('additional_shipping_items.date', '>=', $start)
                        ->whereDate('additional_shipping_items.date', '<=', $end);
        }
    ...
    /**
     * Query to get Voucher details for a customer || supplier
     *
     */
    private function __VoucherQuery($contact_id, $start, $end = null)
    {
     
        $business_id = request()->session()->get('user.business_id');
        $ac_id       = PaymentVoucher::where("account_type",0)->pluck('id');
        $query = PaymentVoucher::leftJoin(
                    'contacts as c',
                    'payment_vouchers.contact_id',
                    '=',
                    'c.id'
                )->where('c.id', $contact_id)->where('payment_vouchers.account_type', 0);


        if (!empty($start)  && !empty($end)) {
            $query->whereDate('payment_vouchers.date', '>=', $start)
                        ->whereDate('payment_vouchers.date', '<=', $end);
        }
     ...
     *
     */
    private function __VoucherQuery2($contact_id, $start, $end = null)
    {
     
        $business_id = request()->session()->get('user.business_id');
        $ac_id       = PaymentVoucher::where("account_type",1)->pluck('id');
        $query = PaymentVoucher::leftJoin(
                    'accounts as c',
                    'payment_vouchers.contact_id',
                    '=',
                    'c.id'
                )->where('c.contact_id', $contact_id)->where("payment_vouchers.account_type",1);
    ...
    public function getProfitLossDetails($business_id, $location_id, $start_date, $end_date, $user_id = null,$reportSetting=null,$loc=null)
    {
        ...
        if($loc==null){ 
            ...
            $day_before_start_date = $start_date;
        }
        
        $filters = ['user_id' => $user_id];
        //Get Opening stock
        $opening_stock = $this->getOpeningClosingStock($business_id, $day_before_start_date, $location_id, true, false, $filters,$loc);
        ...
        $transaction_types = [
            'purchase_return', 'sell_return', 'expense', 'stock_adjustment', 'Stock_Out', 'purchase', 'sale'
        ];
        ...
        // ...... Eb ......  \\
        $opening_s = $this->getTotalByType("opening",$reportSetting,$start_date,$end_date);
         
        //Stocks
        $data['opening_stock'] = !empty($opening_s['total']) ? $opening_s['total']->total_amounts : 0;
        ...
        $data['total_purchase_account'] = $purchase_details['accounts'];
        
        $data['total_expenses_account'] = $transaction_totals['expenses_accounts'];
        $data['total_reveneus_account'] = $transaction_totals['reveneus_accounts'];
        ...
        $data['total_sale_account']  = $sell_details['accounts'];
        ...
        $data['total_reveneus'] =  $transaction_totals['total_reveneus'];
    ...
    public function payContact($request, $format_data = true)
    {
        ...
        if (empty($due_payment_type)) {
            $due_payment_type = $contact->type == 'supplier' ? 'purchase' : 'sale';
        }
    ...
    public function addSellReturn($input, $business_id, $user_id, $uf_number = true)
    {
        ...
        $discount = [
                ...
                'discount_amount' => $input['last_dis'] ?? 0
            ];
        ...
        $sell_return_data = [
            ...
            'exchange_price'       => ($input['currency_id']!= null && $input['currency_id_amount'] != 0)?($input['currency_id_amount']):null,
            'amount_in_currency'   => ($input['currency_id']!= null && $input['currency_id_amount'] != 0)?($this->num_uf($input['total_rt_sale']) / $input['currency_id_amount']):null ,
            'tax_amount'           => $this->num_uf($input['tax_amount']),
            'total_before_tax'     => $this->num_uf($input['sub_total_rt_sale']),
            'final_total'          => $this->num_uf($input['total_rt_sale'])
        ];
        
        \App\Models\StatusLive::insert_data_rs($business_id,$sell,"Sales Purchase",$invoice_total['final_total'] );
        ...
        if (empty($sell_return_data['invoice_no']) && empty($sell_return)) {
            # Update reference count
            $ref_count                      = $this->setAndGetReferenceCount('sell_return', $business_id,$sell->pattern_id);
            $sell_return_data['invoice_no'] = $this->generateReferenceNumber('sell_return', $ref_count, $business_id,null,$sell->pattern_id);
        }
        
        if (empty($sell_return)) {
            $sell_return_data['transaction_date']  = $sell_return_data['transaction_date'] ?? \Carbon::now();
            $sell_return_data['store']             = $sell->store;
            ...
            $sell_return_data['document']          = $input['document'];
            $sell_return_data['cost_center_id']    = $sell->cost_center_id;
        ...
        foreach ($product_lines as $product_line) {
            ...
            $returnss[$product_line['sell_line_id']] = $uf_number ? $this->num_uf($product_line['unit_price_']) : $product_line['unit_price_'];
        }
        foreach ($sell->sell_lines as $sell_line) {
            if (array_key_exists($sell_line->id, $returns)) {
                ...
                $diff = $quantity - $quantity_before;
                // \App\Models\WarehouseInfo::update_stoct($sell_line->product_id,$sell_line->store_id,$diff,$sell->business_id);
                // if (isset($input['line_type'])) {
                //      \App\MovementWarehouse::return_sell($sell_line->transaction,$sell_line->product,$sell_line->quantity_returned
                //                                                                 ,$sell_line->store_id,$sell_line);
                //  }else{
                //      if ($diff > 0) {
                //         \App\MovementWarehouse::movemnet_warehouse($sell,$sell_line->product,$diff,$sell_line->store_id,$sell_line,$type="plus");
                //     }elseif($diff < 0){
                //         \App\MovementWarehouse::movemnet_warehouse($sell,$sell_line->product,abs($diff),$sell_line->store_id,$sell_line,$type="decrement");
                //     }
                // }
            }
        }
        
        \App\AccountTransaction::return_sales($sell,$input["last_discount"],$input["total_rt_sale"],$input["sub_total_rt_sale"],$input["tax_amount"]);
        return $sell_return;     
    }
