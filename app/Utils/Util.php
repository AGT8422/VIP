...
class Util
{
    ...
    */
    ...
    public static function calc_percentage_s($number, $percent, $addition = 0)
    {
        return ($addition + ($number * ($percent / 100)));
    }
    ...
    public function orderStatuses($type=null)
    {
        // 'received' => __('lang_v1.received'),
        if($type==null){
            return [  
                    'received' => __('lang_v1.recieved') ,
                    'final' => __('lang_v1.purchase_final'), 
     ...
     * @return int
     */
    public static function setAndGetReferenceCount($type, $business_id = null,$pattern= null)
    {
        if($business_id == null){
    ...
    public static function generateReferenceNumber($type, $ref_count, $business_id = null, $default_prefix = null,$pattern =null)
    {
        ...
        // }
        if($pattern != null){
            $pat = \App\Models\Pattern::find($pattern);
            $prefix = (!empty($pat))?$pat->scheme->prefix:"";
        }else{
            $prefix = "";
        }
        if (!empty($business_id)) {
            ...
            if (session()->has('business') && !empty(request()->session()->get('business.ref_no_prefixes')[$type])) {
                $business = Business::find(request()->session()->get('business.id'));
                $prefixes = $business->ref_no_prefixes;
                $prefix .= !empty($prefixes[$type]) ? $prefixes[$type] : '';
            }
        ...
        if (!in_array($type, ['contacts', 'business_location', 'username' ,'customer' , 'supplier'  ])) {
    ...
    */
    ...
    public function activityLog($on, $action = null, $before = null, $properties = [], $log_changes = true)
    {
        ...
        activity()->performedOn($on)->withProperties($properties)->log($action);
    }
