$(document).ready(function() {
    ...
    $(document).on('click', 'button.default_button', function() {
        var href = $(this).data('href');
    ...
    $(document).on('click', 'button.In_Price_button', function() {
        var href = $(this).data('href');
        $.ajax({
            method: 'GET',
            url: href,
    ...
    $('#type').change(function() {
        if($(this).val() == "combo"){
            $('#sub_unit_ids').html("");
            $('#sub_unit_ids').closest("div").addClass("hide");
        }else{
            $('#sub_unit_ids').closest("div").removeClass("hide");
            get_sub_units();
        }
        var main_id = $('#unit_id').val();
        var list    = JSON.stringify($('#sub_unit_ids').val());
        show_product_type_form(list,main_id);
        setTimeout(function()
        {
            ready();
        },3000);
    });
    $('#sub_unit_ids').change(function() {
        var main_id = $('#unit_id').val();
        var list    = JSON.stringify($(this).val());
        show_product_type_form(list,main_id);
        setTimeout(function()
        {
            ready();
    ...
    $(document).on('click', '#add_variation', function() {
        ...
        $.ajax({
            ...
            data: { row_index: row_index, action: action ,ks:ks,main_id:main_id,list:list},
            dataType: 'html',
            success: function(result) {
                if (result) {
                    // if(ks == 1){
                    //     $('#product_variation_form_part_1  > tbody').append(result);
                    // }else if(ks == 2){
                    //     $('#product_variation_form_part_2  > tbody').append(result);
                    // }else if(ks == 3){
                    //     $('#product_variation_form_part_3  > tbody').append(result);
                    // }
    ...
    if ($('#invoice_text').length) {
        init_tinymce('invoice_text');
    }
    if ($('#purchase_text').length) {
        init_tinymce('purchase_text');
    }
    if ($('#purchase_footer').length) {
        init_tinymce('purchase_footer');
    }
    if ($('#delivery_text').length) {
        init_tinymce('delivery_text');
    }
    if ($('#description_qutation').length) {
        init_tinymce('description_qutation');
    }
    if ($('#quotation_terms').length) {
        init_tinymce('quotation_terms');
    }
    if ($('#product_description').length) {
        init_tinymce('product_description');
    }
...
function get_sub_units() {
    ...
    if($('#sub_unit_ids').is(':visible')){
        ...
        $.ajax({
            ...
            success: function(result) {
                if (result) {
                    $('#sub_unit_ids').closest(".sub_unit_box").find("label").html() ;
                    $('#sub_unit_ids').closest(".sub_unit_box").find("#sub_unit_ids").html("") ;
                    $('#sub_unit_ids').html(result);
                    var list = JSON.stringify($('#sub_unit_ids').val());
                    show_product_type_form(list,unit_id);
                }else{
                    $('.packages').each(function(){
                         
                        $(this).find("#unit_D").val("")   ;
...
function show_product_type_form(list=null,main_id=null) {
    // #6-8-2024
    ...
    var change     = (list!=null)?"change":"standard"; 
    var main_unit  = (main_id!=null)?"main":"standard"; 
    ...
    $.ajax({
        ...
        dataType: 'html',
        data: { type: $('#type').val(), product_id: product_id, action: action, list:list,change:change,main_id:main_id ,main_unit:main_unit},
        success: function(result) {
            if (result) {
...
function updateProfitLoss(start = null, end = null, location_id = null, selector = null) {
    ...
    var until_date =  $('#until_date').val().format('YYYY-MM-DD');
     
    if(location_id == null){
        var location_id = $('#profit_loss_location_filter').val();
    }
    var data = { start_date: start, end_date: end,until_date: until_date, location_id: location_id };
