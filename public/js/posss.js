...
//&& ui.item.qty_available > 0||(ui.item.enable_stock == 0)
        ...
        $('#search_product')
            .autocomplete({
                source: function(request, response) {
                    ...
                    var quotation = getUrlParameter('status');
                    
                    if($("#status_del").val() == "delivered"){
                        quotation = "delivered";
                    }   
                    if($('#return_type').val() == "return_sale"){
                        return_type = "return_sale";
                    }else{
                        return_type = "";
                    }
                    $.getJSON(
                ...
                select: function(event, ui) {
                    ...
                    // if (ui.item.enable_stock != 1 || is_overselling_allowed || quotation == 'quotation' || quotation == 'draft' || quotation == 'ApprovedQuotation' || quotation == 'proforma' || quotation == 'delivered' || return_type == "return_sale" ) {
                        ...
                        var curr = $(".currency_id_amount").val();
                        pos_product_row(ui.item.variation_id, purchase_line_id,null,1,curr,ui.item.product_id);
                        // update_os();
                        // os_discount();
                        // new_row();
                        // change_currency();
    ...
    $('table#pos_table tbody').on('change', 'input.pos_quantity', function() {
        ...
        $('.row_discount_percentage').each(function(){
            ...
            el.children().find('.unit_price_inc_tax_last_os').val(parseFloat(row_amount_tax).toFixed(2));
        ...
        $('#final_total_input').val(x.toFixed(2));
        $('#amount_0').val(x);
    ...
    $('table#pos_table tbody').on('change', 'input.pos_unit_pricexxxx', function() {
...
function pos_product_row(variation_id = null, purchase_line_id = null, weighing_scale_barcode = null, quantity = 1,curr = null,id_product=null) {
    ...
    if (add_via_ajax) {
        ...
        var contact_id  = $('#con_id').val();
        var return_type = $('#return_type').val();
         
        var  status = 'o';
        
        if(getUrlParameter('status') == "quotation"){
              status = 'quotation';
        ...
        if(getUrlParameter('status') == "ApprovedQuotation"){
        ...
        if(status == "delivered"){
            ...
            status = 'ApprovedQuotation';
        }else if( status == "ApprovedQuotation"  ){
            status = 'ApprovedQuotation';
        ...
        $.ajax({
            method: 'GET',
            url: '/sells/pos/get_product_row/' + variation_id + '/' + location_id + "/" + store_id + "/" + status+ "/"+ contact_id+
                       '?view_type='+view_type  +"&id_product=" + id_product  ,
            async: false,
            data: {
...
function new_row(){
    // alert($("#pos_table tbody .product_row").find(".row_discount_amount").val());
    $("#pos_table tbody .product_row").find(".row_discount_amount").each(()=>{
            var el = $(this);
...
function change_discount() {
    $('.row_discount_percentage').change(function(){
        ...
        if( currancy != "" && currancy >= 0 ){
            var price   =  el.children().find('.pos_unit_price_new_currency').val();
            var amount  =  parseFloat(price)*(dis/100);
            el.children().find('.row_discount_amount').val(amount.toFixed(3));
        }else{
    ...
    $('.row_discount_amount ').change(function(){
        ...
        if( currancy != "" && currancy >= 0 ){
            ...
            el.children().find('.row_discount_percentage').val(amount.toFixed(3));
        }else{
            var price   =  el.children().find('.pos_unit_price').val();
            var amount  =  (dis/price)*100;
            el.children().find('.row_discount_percentage').val(amount.toFixed(3));
    ...
    $('.pos_unit_price').change(function(){
        ...
        var vat    = 0;
        var discount_percentage = el.children().find('.row_discount_percentage').val();
        if ($('#tax_rate_id option:selected').data('rate')) {
            var vat =  parseFloat($('#tax_rate_id option:selected').data('rate'));
        }
        ...
        // console.log(x+'vat'); 
        // console.log(px+'dis');
        el.children().find('.row_discount_amount').val(dap.toFixed(3));
        el.children().find('.inc_vat_p').val(x.toFixed(3));
        el.children().find('.pos_unit_price_os').val(px.toFixed(3));
        el.children().find('.pos_unit_price_inc_tax').val(x.toFixed(3));
        update_os();
        // os_discount();
    })
    $('.pos_unit_price_new_currency').change(function(){
        var  el    =  $(this).parent().parent();
        var price  =  parseFloat($(this).val());
        var vat    =  0;
        var discount_percentage = el.children().find('.row_discount_percentage').val();
        ...
        el.children().find('.row_discount_amount').val(dap.toFixed(2));
    ...
    $('.inc_vat_p').change(function(){
        ...
        var dap = (x * (discount_percentage/100));
        
        el.children().find('.pos_unit_price').val(x.toFixed(2));
        el.children().find('.row_discount_amount').val(dap.toFixed(2));
        update_os();
        // os_discount();
    })
...
$('#tax_rate_id').change(function(){
    var vat = 0;
    if ($('#tax_rate_id option:selected').data('rate')) {
         vat =  parseFloat($('#tax_rate_id option:selected').data('rate')) ;
    }
    $('.pos_unit_price').each(function(){
        var el         =  $(this).parent().parent();
        var unit_price =  parseFloat($(this).val());
        var qty        =  parseFloat(el.children().find('.pos_quantity').val());
        var unit_vat   =  unit_price + unit_price*(vat/100);
        //console.log(unit_vat+'tax page');
        // el.children().find('.inc_vat_p').val(parseFloat(unit_vat).toFixed(3));
        el.children().find('.inc_vat_p').val(unit_vat);
    })
    change_discount();
    update_os();
    os_discount();
...
function change_currency(){
    $('#pos_table .pos_unit_price_new_currency').on("change" ,function(){
        var vl = $(this).val();
        var e  = $(this).parent().parent();
        
        // var el = e.children().find(".purchase_unit_cost_with_tax_new_currency");
        var inc_vat      = e.children().find(".inc_vat_p");
        var exc_vat      = e.children().find(".pos_unit_price");
        var exc_dis_vat  = e.children().find(".pos_unit_price_os");
        var inc_dis_vat  = e.children().find(".unit_price_inc_tax_last_os");
        var tax_amount   = parseFloat($('#tax_rate_id option:selected').data('rate')) ;
        var tax_rate     = parseFloat($('option:selected', $('#tax_rate_id')).data('rate'));
        var dis_amount   = ( parseFloat(e.children().find('.row_discount_amount').val()) > 0 )?parseFloat(e.children().find('.row_discount_amount').val()) :0;
...
function update_os() {
    ...
    $('.row_discount_percentage').each(function(){
        ...
        // .... choose depending on currency discount 
        if(currancy != "" && currancy != 0){/** here if select new currency */
            if (el.children().find('.row_discount_amount').val()) {
               dis      =  parseFloat(el.children().find('.row_discount_amount').val(),10); 
               dis_curr =  dis ;
               dis      =  parseFloat(dis * currancy) ; 
            }
        }else{/** here if none selected currency */
            if (el.children().find('.row_discount_amount').val()) {
               dis =  parseFloat(el.children().find('.row_discount_amount').val(),10); 
            }
        }
        
        el.children().find('.pos_unit_price_os').val((price - dis).toFixed(3));
        var tax_price   = parseFloat(el.children().find('.inc_vat_p').val()) ;
        var qty         = parseFloat(el.children().find('.pos_quantity').val(),10) ;
        var row_amount  =  ((price - dis)*qty).toFixed(3);
        var after_discount_tax =  parseFloat(el.children().find('.pos_unit_price_os').val());
      
        if(currancy != "" && currancy != 0){
            el.children().find('.pos_unit_price_new_currency').val((price/currancy).toFixed(2));
            el.children().find('.inc_vat_p_new_currency').val((tax_price/currancy).toFixed(2));
            var price_curr         =  parseFloat(el.children().find('.pos_unit_price_new_currency').val(),10);
            var row_amount_curr    = ((price_curr - dis_curr)*qty).toFixed(3);
        }else{
        ...
        amount              =  parseFloat(amount) + parseFloat(row_amount) ;
        amount_curr         =  parseFloat(amount_curr) + parseFloat(row_amount_curr) ;

        var old_price       =  parseFloat(after_discount_tax,3);
        var vat_line        =  parseFloat((after_discount_tax * vat / 100),3);
        with_vat            =  old_price  +  vat_line ;

        el.children().find('.unit_price_inc_tax_last_os').val(with_vat.toFixed(3));
        el.children().find('.pos_line_total_text').text((row_amount_tax*qty).toFixed(3));
        el.children().find('.pos_line_total').val((row_amount));
        total_quantity += qty;
        if(currancy != "" && currancy != 0){
            el.children().find('.pos_unit_price_os_new_currency').val((parseFloat(el.children().find('.pos_unit_price_new_currency').val())-parseFloat(dis_curr)));
            var after_discount_tax_curr = parseFloat(el.children().find('.pos_unit_price_os_new_currency').val());
            var vat_line_curr        =  parseFloat((after_discount_tax_curr * vat / 100),3);
            with_vat_curr            =  after_discount_tax_curr  +  vat_line_curr ;
            el.children().find('.unit_price_inc_tax_last_os_new_currency').val(parseFloat(with_vat_curr).toFixed(2));
        }else{
    ...
    amount_curr   =  parseFloat(amount_curr,10);

    $('.price_total').text(amount.toFixed(2));
    $('.price_total_curr').text(amount_curr.toFixed(2));

    var tax       =  parseFloat((vat/100)*amount,10);
    var tax_curr  =  parseFloat((vat/100)*amount_curr,10);

    $('#order_tax').text(tax.toFixed(3));
    $('#tax_calculation_amount').val(tax);
  
    currancy = $(".currency_id_amount").val();
    if(currancy != "" && currancy != 0){
        $('#order_tax_curr').text(tax_curr.toFixed(2));
    }
    
    
    var x      =   parseFloat(amount,10) + parseFloat(tax,10) ;
    var x_curr =   parseFloat(amount_curr,10) + parseFloat(tax_curr,10) ;

    $('#total_payable').text(x.toFixed(2));
    if(currancy != "" && currancy != 0){
        $('#total_payable_curr').text(x_curr.toFixed(2));
    }
...
//         var row_amount_tax  =  ( after_discount_tax + after_discount_tax*(vat/100));
//         amount = parseFloat(amount) + parseFloat(row_amount) ;
//         el.children().find('.unit_price_inc_tax_last_os').val((row_amount_tax).toFixed(2));
//         el.children().find('.pos_line_total_text').text((row_amount_tax*qty)+' AED');
//         el.children().find('.pos_line_total').val((row_amount));
//         total_quantity += qty;
...
// }
     $('.row_discount_percentage').each(function(){
        var el         =  $(this).parent().parent();
        var price      =  parseFloat(el.children().find('.pos_unit_price_new_currency').val(),3);
        var old_price  =  parseFloat((price*currancy).toFixed(3)).toFixed(3);
        console.log("#one " + old_price);
        var vat_line   =  parseFloat((old_price * vat / 100).toFixed(3));
        with_vat       =  parseFloat(old_price)  +  parseFloat(vat_line) ;
        el.children().find('.pos_unit_price').val((price*currancy).toFixed(3));
        el.children().find('.inc_vat_p').val(parseFloat(with_vat).toFixed(3));
     });
    update_os();
}

if ($('#sell_proccess').length ) {
...
function update_list_sell() {
    $('.product_id').each(function(){
        var product_id = $(this).val();
        var status     = $('#status option:selected').val();
        var store_id   = $('#store_id option:selected').val();
        let el =  $(this);
        // $.get('/pos-sell-stock/'+product_id+'/'+status,{store_id:store_id},function(data){
        //     let  parent = el.parent().parent();
        //     if (data < 1) {
        //         el.parent().parent().remove();
        //     }else{
        //         let no  = parseInt(data) ;
        //         parent.children().find('.input_quantity').attr('max',no);
        //         parent.children().find('.input_quantity').attr('data-rule-max-value',no);
        //         parent.children().find('.input_quantity').attr('data-qty_available',no);
                
        //         if (parseInt(parent.children().find('.input_quantity').val())  > no) {
        //             parent.children().find('.input_quantity').val(no);
        //         }
        //     }
        // })
    })
...
function pos_total_row() {
    ...
    $('span.total_quantity_').each(function() {
        $(this).html(__number_f(total_quantity));
    });
...
function calculate_billing_details(price_total) {
    ...
    //......... * change wrong amount get
    var total_payable = parseFloat(price_total).toFixed(2) + parseFloat(order_tax).toFixed(2) - parseFloat(discount).toFixed(2) + parseFloat(shipping_charges).toFixed(2) + parseFloat(packing_charge).toFixed(2);
    ...
    // console.log("###total payable 5 - " +  shown_total )
    $('span#total_payable').text(__currency_trans_from_en(shown_total, false));
    currancy = $(".currency_id_amount").val();
    var currancy_final = (shown_total/currancy).toFixed(2);
    $('span#total_payable_curr').text(__currency_trans_from_en(currancy_final, false));
...
function pos_discount(total_amount) {
    ...
    if(currany != "" && currancy >= 0){
        $('span#total_discount').text(__currency_trans_from_en((discount*currancy).toFixed(3), false));
        $('span#total_discount_curr').text(__currency_trans_from_en(discount.toFixed(2), false));
    }else{
        $('span#total_discount').text(__currency_trans_from_en(discount, false));
        $('span#total_discount_curr').text(__currency_trans_from_en((discount/currany).toFixed(4), false));
    }
    return discount;
}
...
function pos_order_tax(price_total, discount) {
    ...
    var tax_for_use_later  =  $('#tax_rate_id option:selected').data('rate');
    var calculation_type   = 'percentage';
    var calculation_amount =  $('#tax_rate_id option:selected').data('rate');
    ...
    $('span#order_tax').text( parseFloat(order_tax).toFixed(3)  );
...
$(document).on('change', '#discount_type_modal, #discount_type, #discount_amount', function() {
    os_discount();
});
...
function os_discount() {
    ...
    if (type == 'fixed_before_vat') {
        ...
        var price_total_curr =  $('.price_total_curr').text();
        var order_tax_curr   =  (price_total_curr - discount_amount)*(vat/100);
        var order_tax        =  (parseFloat(sub_total).toFixed(3) - parseFloat(discount_amount).toFixed(3))*(vat/100);
        // currancy = $(".currency_id_amount").val();
        // var order_tax   =  (sub_total - discount_amount)*(vat/100);
       
        if(currancy != "" && currancy != 0){
            var order_tax        =  (parseFloat(sub_total).toFixed(3) - parseFloat(discount_amount*currancy).toFixed(3))*(vat/100);
            $('#total_discount').text((discount_amount*currancy).toFixed(3));
            $('#total_discount_curr').text(parseFloat(discount_amount).toFixed(2));
            $('#order_tax_curr').text(order_tax_curr.toFixed(2));
        }
        ...
        vat_global  =  $('#tax_rate_id option:selected').data('rate');
        if(vat_global == null){vat_global = 0;}
        
        var x =  (discount_amount*100)/(100+parseFloat(vat_global));
        $('#total_discount').text(x.toFixed(3));

        var order_tax   =  (sub_total - x)*(vat/100);
        
        if(currancy != "" && currancy != 0){
            var price_total_curr =  $('.price_total_curr').text();
            var order_tax_curr   =  (price_total_curr - x)*(vat/100);
            order_tax        =  (sub_total - x*currancy)*(vat/100);
            $('#total_discount').text((x*currancy).toFixed(3));
            $('#total_discount_curr').text((x).toFixed(2));
            $('#order_tax_curr').text(order_tax_curr.toFixed(2));
        }

        $('#order_tax').text(order_tax.toFixed(2));
        $('#tax_calculation_amount').val(order_tax);
        // console.log(order_tax+'_______')
        $('#endregionorder_tax').text(order_tax.toFixed(3));
        discount_amount =  x; 
        ...
        // currancy = $(".currency_id_amount").val();
        if(currancy != "" && currancy != 0){
            var x_curr =  price_total_curr*(discount_amount/100);
            var order_tax_curr   =  (price_total_curr - x_curr)*(vat/100);
            $('#total_discount_curr').text(parseFloat(x_curr).toFixed(2));
            $('#order_tax_curr').text(parseFloat(order_tax_curr).toFixed(2));
        }
        $('#order_tax').text(order_tax);
        $('#tax_calculation_amount').val(order_tax);
        // console.log(order_tax+'___55555____');
        $('#total_discount').text(x.toFixed(3));
        discount_amount =  x; 
    }
    ...
    if (type && $('#discount_amount').val() > 0 ) {
        ...
        if(currancy != "" && currancy != 0){
             var price_total_curr =  $('.price_total_curr').text();
             var price_total_dis_curr =  $('#total_discount_curr').text();
             var price_total_vat_curr =  $('#order_tax_curr').text();
             final_curr = parseFloat(price_total_curr)  -  parseFloat(price_total_dis_curr)  +  parseFloat(price_total_vat_curr) ;
             $('#total_payable_curr').text(parseFloat(final_curr).toFixed(2));
             final =  sub_total -  parseFloat($("#total_discount").text()) + parseFloat($("#order_tax").text()) + parseFloat(ship);
        }
        $('#total_payable').text(final.toFixed(3));
        $('#final_total_input').val(final.toFixed(3));
        $('#amount_0').val(final);  
    }else{
        var ship = $('#shipping_charges').val();
        if(ship == ""){
        ...
        if(currancy != "" && currancy != 0){
            var price_total_curr     =  $('.price_total_curr').text();
            var price_total_dis_curr =  $('#total_discount_curr').text();
            $('#order_tax_curr').text(parseFloat(order_tax/currancy).toFixed(3));
            var price_total_vat_curr =  $('#order_tax_curr').text();
            final_curr               =  parseFloat(price_total_curr)  - parseFloat(price_total_dis_curr) + parseFloat(price_total_vat_curr) ;
            $('#total_payable_curr').text(parseFloat(final_curr).toFixed(2));
            final                    =  sub_total  + parseFloat($("#order_tax").text()) + parseFloat(ship);
        }else{
            $('#order_tax').val(order_tax.toFixed(3));
    ...
    $('#order_tax').val(order_tax.toFixed(3));
    $('#tax_calculation_amount').val(order_tax.toFixed(3));
