$(document).ready(function() {
        ...
        $('#search_product_for_srock_adjustment')
            .autocomplete({ 
                source: function(request, response) {
                    $.getJSON(
                        '/products/list', 
                        { location_id: $('#location_id1').val(), term: request.term },
                        response
                    );
                },
                minLength: 2,
                        ...
                        if (ui.item.qty_available > 0 && ui.item.enable_stock == 1) {
                             if($(".product_id").val()){
                                    $(".product_id").each(function(){
                                        ...
                                        if(ui.content[0].product_id == e.find(".product_id").val() ){
                                            e.find(".input_quantity").val(parseFloat(e.find(".input_quantity").val()) + 1 );
                                            checks = 1;
                                        } 
                                });
                            } 
                ...
                select: function(event, ui) {
                    ...
                    if (ui.item.qty_available > 0) {
                        ...
                        if($(".product_id").val()){
                            $(".product_id").each(function(){
                                var e  = $(this).parent().parent();
                                if(ui.item.product_id == e.find(".product_id").val() ){
                                    e.find(".input_quantity").val(parseFloat(e.find(".input_quantity").val()) + 1 );
                                    checks = 1;
                                } 
                        ...
                        if(checks == 0){
                            stock_transfer_product_row(ui.item.variation_id,ui,ui.item);
                        }
    ...
    $('form#stock_transfer_form').validate({
        rules: {
            transfer_location_id: {
                notEqual: function() {
                    return $('select#location_id1').val();
                },
            },
        },
    });
    ...
    function up(){
        ...
        $('table#stock_adjustment_product_table tbody tr').each(function() {
            var e   = $(this).find(".input_quantity");
                el += parseFloat(e.val()); 
        });
        $("#total_adjustment_").text(el);
    ...
    stock_transfer_table = $('#stock_transfer_table').DataTable({
        ...
        columns: [
            ...
            { data: 'shipping_charges', name: 'shipping_charges' ,class:'hide_td'},
...
function stock_transfer_product_row(variation_id,ul,item) {
    var row_index = parseInt($('#product_row_index').val());
    var location_id = $('select#location_id1').val();
    var store_id = $('select#location_id').val();
    var location_id_old = $('#location_id_old').val();
     var transfer_location_id = $('select#transfer_location_id').val();
    var status = "transafer";
    $.ajax({
        method: 'POST',
        url: '/stock-adjustments/get_product_row/' + variation_id + '/' + location_id +'/'+ store_id + '/' + status   ,
        data: { row_index: row_index,  variation_id: variation_id, location_id: location_id ,   store_id: store_id , transfer_location_id : transfer_location_id},
        dataType: 'html',
        success: function(result) { 
            if(result==""){
