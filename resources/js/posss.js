            ...
            .autocomplete({
                ...
                select: function(event, ui) {
                    ...
                    if (ui.item.enable_stock != 1 || ui.item.qty_available > 0 || is_overselling_allowed || quotation == 'quotation' || quotation == 'draft' || quotation == 'ApprovedQuotation' || quotation == 'proforma' || quotation == 'delivered' || return_type == "return_sale" ) {
                        ...
                        pos_product_row(ui.item.variation_id, purchase_line_id,null,1,curr);
                        update_os();
                        os_discount();
                        // new_row();
                        change_currency();
...
function pos_product_row(variation_id = null, purchase_line_id = null, weighing_scale_barcode = null, quantity = 1,curr = null) {
...
function change_discount() {
    ...
    $('.pos_unit_price').change(function(){
        ...
        el.children().find('.inc_vat_p').val(x);
        el.children().find('.pos_unit_price_os').val(px);
        el.children().find('.pos_unit_price_inc_tax').val(x);
        update_os();
        os_discount();
    })
...
function update_os() {
    ...
    $('#total_payable').text(x.toFixed(2));
    if(currancy != "" && currancy != 0){
        $('#total_payable_curr').text((x.toFixed(2)/currancy).toFixed(4));
    }
    $('#final_total_input').val(x);
    $('#amount_0').val(x);
    $('.total_quantity').text(total_quantity);
...
function calculate_billing_details(price_total) {
    ...
    $('span#total_payable').text(__currency_trans_from_en(shown_total, false));
    currancy = $(".currency_id_amount").val();
    var currancy_final = (shown_total/currancy).toFixed(4);
...
function os_discount() {
    ...
    if (type == 'fixed_before_vat') {
        ...
        currancy = $(".currency_id_amount").val();
        if(currancy != "" && currancy != 0){
            $('#total_discount_curr').text((discount_amount/currancy).toFixed(4));
            $('#order_tax_curr').text((order_tax.toFixed(3)/currancy).toFixed(4));
        }
        ...
        var x =  (discount_amount*100)/105;
        $('#total_discount').text(x.toFixed(2));
    ...
    if (type && $('#discount_amount').val() > 0 ) {
        ...
        $('#total_payable').text(final.toFixed(2));
        currancy = $(".currency_id_amount").val();
        if(currancy != "" && currancy != 0){
              $('#total_payable_curr').text((final.toFixed(2)/currancy).toFixed(4));
        }
        $('#final_total_input').val(final);
        $('#amount_0').val(final);  
    }else{
        var ship = $('#shipping_charges').val();
        if(ship == ""){
