...
// ** 2
    ...
    var tax_amount  = parseFloat($('#tax_id option:selected').data('tax_amount')) ;
    var el          =  $(this).parent().parent();
    el.children().find('.purchase_unit_cost_without_discount').val( parseFloat($(this).val()));
    var price       =   parseFloat($(this).val())*(tax_amount/100) + parseFloat($(this).val()) ; 
    el.children().find('.purchase_unit_cost_with_tax').val(price.toFixed(2));
    var intial_avl =  parseFloat($(this).val());
        os_total_sub();
...
// ** 1
    ...
    $("#total_final_").html((parseFloat(total) + parseFloat(ship) + parseFloat(ship_)).toFixed(2));       
...
// ** 9
   ...
   $('.purchase_quantity').each(function(){
    ...
    $('#total_subtotal_input').val(total_amount);
    $('#total_subtotal').text(total_amount);
    ...
    if(dis_type) {
        if (dis_type.length > 0) {
            ...
            if (dis_type ==  'fixed_before_vat') {
                ...
                if(currancy != "" && currancy != 0){
                    $('#discount_calculated_amount_cur').text((dis/currancy).toFixed(4));
                }
                ...
                if(currancy != "" && currancy != 0){
                    $('#discount_calculated_amount_cur').text((x.toFixed(2)/currancy).toFixed(4));
                }
            }else{
...
// ** 11
    ...
    currancy = $(".currency_id_amount").val();
    if(currancy != "" && currancy != 0){
        $('#tax_calculated_amount_curr').html((x.toFixed(2)/currancy).toFixed(4));
    }
     $('#tax_amount').val(x);
    var shipping   =  0;
    ...
    // var grand     =  net + ((vat_per/100)*net) + shipping - dis_count;
     var grand     =  parseFloat(net) + parseFloat(((vat_per/100)*net)) + parseFloat(shipping) - parseFloat(dis_count);
    $('#grand_total').text(grand.toFixed(3));
    currancy = $(".currency_id_amount").val();
    if(currancy != "" && currancy != 0){
        $("#total_final_i_curr").html(((parseFloat(grand).toFixed(2))/currancy).toFixed(4));       
         
    }    
    $('#grand_total_hidden').text(grand.toFixed(3));
    $('#grand_total_hidden').val(grand.toFixed(3));

    $('#grand_total2').text(grand.toFixed(3));
...
// ** 15
    ...
    $('.purchase_unit_cost').each(function () {
        ...
        currancy = $(".currency_id_amount").val();
        if(currancy != "" && currancy != 0){
            var pp_price_currency      =  parseFloat(el.children().find('.purchase_unit_cost_new_currency').val((pp_price/currancy).toFixed(4)));
            var tax_price_currency     =  parseFloat(el.children().find('.purchase_unit_cost_with_tax_new_currency').val((tax_price/currancy).toFixed(4)));
        }else{
        ...
        el.children().find('.purchase_unit_cost').val(final_amount.toFixed(4));
        el.children().find('.row_total_cost').val(parseInt(final_amount*quantity).toFixed(4));
        
        var before_tax_dis = el.children().find('.total_cost_dis_new_currency'); 
        var before_tax_dis_edit = el.children().find('.unit_cost_after_new_currency'); 
        ...
        el.children().find('.total_unit_cost_with_tax').val(total_unit_cost_with_tax.toFixed(4));
        if(currancy != "" && currancy != 0){
            var pp_price_dis_currency      =  parseFloat(el.children().find('.total_cost_dis_new_currency').val((final_amount.toFixed(4)/currancy).toFixed(4)));
            var tax_price_dis_currency     =  parseFloat(el.children().find('.total_unit_cost_with_tax_new_currency').val((total_unit_cost_with_tax.toFixed(4)/currancy).toFixed(4)));
            var tax_price_edit             =  parseFloat(el.children().find('.unit_cost_after_new_currency').val((final_amount.toFixed(4)/currancy).toFixed(4)));
            var tax_price_edit_currency    =  parseFloat(el.children().find('.unit_cost_after_tax_new_currency').val((total_unit_cost_with_tax.toFixed(4)/currancy).toFixed(4)));
            el.children().find('.row_total_cost_new_currency').val((parseInt(total_unit_cost_with_tax*quantity).toFixed(4)/currancy).toFixed(4));
        }else{
      ...
      //  el.children().find('.purchase_unit_cost_with_tax').val(total_unit_cost_with_tax.toFixed(3));
        ...
        console.log(out_tax_amount_cur_edit + "sssss");
    ...
    $('#total_subtotal').html(out_tax_amount.toFixed(4));
    $('#total_subtotal_input').val(out_tax_amount.toFixed(4));
    $('#total_subtotal_cur').html(out_tax_amount_cur.toFixed(4));
    $('#total_subtotal_cur_edit').html(out_tax_amount_cur_edit.toFixed(4));
    $('#total_subtotal_input_cur').val(out_tax_amount_cur.toFixed(4));
    $('#tax_calculated_amount').html(tax_calculated_amount.toFixed(2));
    currancy = $(".currency_id_amount").val();
    if(currancy != "" && currancy != 0){
        $('#tax_calculated_amount_curr').html((tax_calculated_amount.toFixed(2)/currancy).toFixed(4));
    }
