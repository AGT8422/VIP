...
<section class="content no-print font_text">
    ...
    <input type="hidden" id="sell_list_filter_customer_id" value="{{$id}}">
    <input type="hidden" id="purchase_list_filter_supplier_id" value="{{$id}}">
    <br>
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="nav-tabs-custom">
                <ul class="nav nav-tabs nav-justified">
                    ...
                    @if(in_array($contact->type, ['both', 'customer']))
                        <li class="
                            @if(!empty($view_type) &&  $view_type == 'report')
                                active
                            @else
                                ''
                            @endif">
...
$(document).ready( function(){
    ...
    rp_log_table = $('#rp_log_table').DataTable({
        ...
        serverSide: true,
        aaSorting: [[0, 'desc']],
        ajax: '/sells?customer_id={{ $id }}&rewards_only=true',
        columns: [
...
function get_contact_payments(url = null) {
    if (!url) {
        url = "{{action('ContactController@getContactPayments', [$id])}}";
    }
    $.ajax({
        url: url,
        dataType: 'html',
...
function get_contact_ledger() {
    ...
    $.ajax({
        url: '/contacts/ledger?contact_id={{$id}}&start_date=' + start_date + '&transaction_types=' + transaction_types + '&show_payments=' + show_payments + '&end_date=' + end_date,
        dataType: 'html',
        success: function(result) {
            $('#contact_ledger_div')
                .html(result);
...
@include('sale_pos.partials.subscriptions_table_javascript', ['contact_id' => $id])
@endsection
