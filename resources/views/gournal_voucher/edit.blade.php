...
<section class="content">
	{!! Form::open(['url' => 'gournal-voucher/edit/'.$data->id, 'method' => 'post', 'id' => 'daily_payment_form', 'files' => true ]) !!}
			...
			<div class="row">
				<div class="col-md-3">
					<div class="form-group"> 
						...
						{{ Form::select('main_account_id',$accounts,$data->main_account_id,['class'=>'form-control select2'
						     ,'id'=>'main_account_id','placeholder'=>trans('home.please account'),'required']) }}
					</div>
				</div>
				<input type="hidden" id="list_account_checked" @if($data->main_credit == 1)    value="1" @else value="0" @endif >
				<input type="hidden" id="list_account" data-list="{{json_encode($accounts)}}">
				<div class="col-sm-3">
					<div class="form-group">
						<div class="checkbox">
							 
							{!! Form::checkbox('main_credit',    1  , $data->main_credit , [ 'class' => 'input-icheck' , 'id' => 'main_credit']); !!} {{ __( 'home.Main Credit' ) }} 
							 <br>
							 @if($data->main_credit == 1)
							 {!! Form::text("total_credit",0,['class'=>'form-control','readOnly','id'=>'total_credit']) !!}
							 @else
				...
				<div class="col-md-3">
					<div class="form-group">
						...
						{!! Form::date('gournal_date',$data->date, ['class' => 'form-control ' ,'required']); !!}
					</div>
				</div>
				{{-- <div class="col-md-2">
					<div class="form-group">
				...
				<div class="col-md-2">
					<div class="form-group">
						...
						{{ Form::select('cost_center_id',$cost_centers,$data->cost_center_id,['class'=>'form-control select2'
				...
				<div @if($data->main_credit == 1)  class="main_note col-sm-12  " @else class="main_note col-sm-12 hide"  @endif >
					<div class="form-group">
						...
						@foreach ($data->items as $key=>$item)
						   <tr class="rows">
							<td class="col-xs-2 crd_account">
								...
								{{ Form::select('old_credit_account_id[]',$accounts,$item->credit_account_id,
								['class'=>'form-control select2 credit_account_id','placeholder'=>trans('home.please account')]) }}
							  </td>
							  <td class="col-xs-2">
								{{ Form::select('old_debit_account_id[]',$expenses,$item->debit_account_id,['class'=>'form-control select2 debit_account_id','placeholder'=>trans('home.please account'),'required']) }}
							  </td>
							  <td class="col-xs-1">
								{{ Form::number('old_amount[]',$item->amount,['class'=>'form-control amount','required','step'=>'any','min'=>1,'onKeyUp'=>"UpdateItem(this)"]) }}
							  </td>
							  <td class="col-xs-2">
								{{ Form::select('old_center_id[]',$cost_centers,$item->cost_center_id,['class'=>'form-control select2 center_id','placeholder'=>trans('home.please account')]) }}
							  </td>
							  
							  <td class="col-xs-1">
								{{ Form::number('old_tax_percentage[]',$item->tax_percentage,['class'=>'form-control tax_percentage','required','step'=>'any','min'=>0,'onKeyUp'=>"UpdateItem(this)"]) }}
							  </td>
							  <td class="col-xs-1">
								{{ Form::number('old_tax_amount[]',$item->tax_amount,['class'=>'form-control tax_amount','required','step'=>'any','min'=>0,'readOnly']) }}
							  </td>
							  <td class="col-xs-1">
								{{ Form::number('old_net_amount[]',($item->amount - $item->tax_amount),['class'=>'form-control net_amount','required','step'=>'any','min'=>0,'readOnly']) }}
							  </td>
							  <td class="col-xs-1 hide">
								{{ Form::date('old_date[]',$item->date,['class'=>'form-control ','required']) }}
							  </td>
							  <td class="col-xs-2">
	...
	function UpdateItem(el) {
		...
		function change_dropdown(check=null){ 
			...
			if(check!=null){
				...
				$(".old_credit_account_id").each(function(){
					list_account = JSON.stringify($('#list_account').data('list'));
					list         = JSON.parse(list_account);
					html         = '<option value=""  selected>please select account</option>';
					for(e in list){
