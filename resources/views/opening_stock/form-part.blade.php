				...
				<div class="box-body">
					<div class="row">
						<div class="col-sm-12">
										...
										@foreach($product->variations as $variation)
											...
											@foreach($purchases[$key][$variation->id] as $sub_key => $var)
												@php
													...
													if(count($var) > 4 && isset($var['store_id'])){
												...
												<tr>
													...
													<td>
														{!! Form::text('stocks[' . $key . '][' . $variation->id . '][' . $sub_key . '][purchase_price]', @num_format($purcahse_price) , ['class' => 'form-control input-sm input_number unit_price', 'required' , "readonly" => "true"]); !!}
													</td>

													@if($enable_expiry == 1 && $product->enable_stock == 1)
														<td>
															{!! Form::text('stocks[' . $key . '][' . $variation->id . '][' . $sub_key . '][exp_date]', !empty($var['exp_date']) ? @format_date($var['exp_date']) : null , ['class' => 'form-control input-sm os_exp_date', 'readonly', "readonly" => "true"]); !!}
														</td>
													@endif

													@if($enable_lot == 1)
														<td>
															{!! Form::text('stocks[' . $key . '][' . $variation->id . '][' . $sub_key . '][lot_number]', $lot_number , ['class' => 'form-control input-sm', "readonly" => "true"]); !!}
														</td>
													@endif
													...
													@if($store_id != null)
															<td>
																<div class="form-group">
																	{{-- {!! Form::label('stocks[' . $key . '][' . $variation->id . '][' . $sub_key . '][store_id]', __('warehouse.warehouse').':*') !!} --}}
																	{{-- @show_tooltip(__('tooltip.purchase_location')) --}}
																	{!! Form::text('stocks[' . $key . '][' . $variation->id . '][' . $sub_key . '][store_id]', $mainstore_categories[$store_id], ['class' => 'form-control select2',   'required', "readonly" => "true"], $bl_attributes); !!}
																</div>
															</td>
													@else
														<td>
															<div class="form-group">
																{{-- {!! Form::label('stocks[' . $key . '][' . $variation->id . '][' . $sub_key . '][store_id]', __('warehouse.warehouse').':*') !!} --}}
																{{-- @show_tooltip(__('tooltip.purchase_location')) --}}
																{!! Form::select('stocks[' . $key . '][' . $variation->id . '][' . $sub_key . '][store_id]', $mainstore_categories,null, ['class' => 'form-control select2',   'required', "readonly" => "true"], $bl_attributes); !!}
															</div>
														</td>
													@endif
													...
													<td class="hide">
														@if($loop->index == 0)
															<button type="button" class="btn btn-primary btn-xs add_stock_row" data-sub-key="{{ count($purchases[$key][$variation->id])}}" 
																data-row-html='<tr class="rw" data-id="__subkey__">
																	<td>
																		{{ $product->name }} @if( $product->type == "variable" ) (<b>{{ $variation->product_variation->name }}</b> : {{ $variation->name }}) @endif
																	</td>
																	<td>
																...
																@endif
																<td>
																	{!! Form::select('stocks[' . $key . '][' . $variation->id . '][__subkey__][store_id]', $mainstore_categories,null, ['class' => 'form-control input-sm select2',   'required'], $bl_attributes); !!}
																...
																<td>
																	<div class="delete_stock_row" data-id="__subkey__"><i class="fa fa-minus"></i></div>
																</td>
																<td>&nbsp;</td>
							...
							<div style="border:1px solid black;border-radius:.3rem; width:100px;padding:10px" class="close_btn">@lang("messages.close")</div>
						</div>
					</div>
				</div>
			</div> <!--box end-->
