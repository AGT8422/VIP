...
<head>
    ...
    <title>@lang("home.Cheque_in")</title>
    ...
    <div class="bill"  >
        ...
        @php  if(!empty($payment))  { $trans  = \App\Transaction::where("id",$payment->transaction_id)->first(); }   @endphp
            <div style="font-size: 12px;width:70%">
                 <b>- {{($invoice->account_type != 0)? "Account":"Customer"}}  Name :</b> <span>{{($invoice->account_type == 0)?$invoice->contact->name:$invoice->contacts->name}}</span> <br>
            ...
            <div style="border-bottom:2px solid #8e0f82">&nbsp;</div>
            <div>&nbsp;</div>
