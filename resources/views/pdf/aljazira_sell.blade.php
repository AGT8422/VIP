...
<body>
    ...
    <div class="bill" >
        ...
        <table style="width:100%;margin-bottom:0px;margin-top:-10px;  ">
            <thead>
                <tr>
                <td style="width:100%;border-bottom:0px solid #5a5f54;text-align:center;margin-top:-200px;background-color:black;" >
                    <img src="{{URL::to("http://aljazira.izocloud.com/uploads/img/aljazira.jpeg")}}"   style="max-width: 100%;max-height:220px">
                </td>
            ...
            <tbody>
                ...
                <td style="width: 40%;text-align:center;color:#fff;text-align:center;padding-right:30px;">
                    ...
                    <td style=" width:40%;line-height:20px;font-size:16px;">
                        <h1 style=" width:100% !important;      text-align:right !important;  text-transform: capitalize; ">
    ...
    <!--  -->
        ...
        <table class="table" style="width:100%;margin-top: 30px;text-align:left;border: 1px solid; border-radius:.3rem"  dir="ltr" >
            <thead>

                <tr>
                    <th style="font-size:12px;font-weight: bold;background-color:#000;color:#fff;width:10%">{{ trans('home.NO') }}</th>
                    @php $se = 0; @endphp
                    @foreach($allData as $data) @if(($data->se_note != null && $data->se_note !="")  ) @php $se = 1; @endphp @endif  @endforeach
                    @if($se != 0)    
                    <th style="font-size:12px;font-weight: bold;background-color:#000;color:#fff;width:10%">{{ trans('home.SE') }}</th>
                    @endif
                    <th style="font-size:12px;font-weight: bold;background-color:#000;color:#fff;width:10%">{{ trans('product.sku') }}</th>
                    @if($invoice->sub_status == 'final' || $invoice->sub_status == 'delivered' || $invoice->sub_status == 'f' )
                    @else
                        <th style="font-size:12px;font-weight: bold;background-color:#000;color:#fff;width:15%">{{ trans('home.PHOTO') }}</th>
                    @endif
                    <th style="font-size:12px;font-weight: bold;background-color:#000;color:#fff;width:30%">{{ trans('home.Product') }}</th>
                    <th style="font-size:12px;font-weight: bold;background-color:#000;color:#fff;width:30%">{{ trans('home.DESCRIPTION') }}</th>
                    <th style="font-size:12px;font-weight: bold;background-color:#000;color:#fff;width:5%">{{ trans('home.QTY') }}</th> 
                    ...
                    @if($dis != 0)
                    <th style="font-size:12px;font-weight: bold;background-color:#000;color:#fff;width:5%">Tot Before Dis</th> 
                    <th style="font-size:12px;font-weight: bold;background-color:#000;color:#fff;width:5%">Dis</th> 
                    @endif
                    <th style="font-size:12px;font-weight: bold;background-color:#000;color:#fff;width:5%">Unit Cost Exc.vat</th> 
                    <!--<th style="font-size:12px;font-weight: bold;background-color:#000;color:#fff">Unit Cost Inc.vat	</th> -->
                    <th style="font-size:12px;font-weight: bold;background-color:#000;color:#fff;width:5%">Subtotal	</th> 
            ...
            <tbody>
                ...
                @foreach ($allData as $data)
                        ...
                        ?>
                        <tr>
                            <td style="font-size:16px;width:5%">{{ $count++ }}</td>
                            @if($data->se_note != null && $data->se_note != "")
                                 <td style="font-size:16px;width:5%">{{ $data->se_note }}</td>
                            @endif
                            <td style="font-size:16px;width:5%">{{ $data->product->sku }}</td>
                            ...
                            @else
                                <td style="font-size:16px;width:15%">
                                    ...
                                    <img src="{{ URL::to($data->product->image_url) }}" style="max-width: 10%"> 
                                    @endif
                                </td>
                            @endif
                            ...
                            <td style="font-size:14px;width:40%">{{ strip_tags($data->sell_line_note) }}</td>
                            <td style="font-size:16px;width:5%">{{ $data->quantity }}</td>
                            ...
                            @if($discount!=0)
                                <td style="font-size:16px;width:5%">@format_currency($data->unit_price_before_discount)</td> 
                                <td style="font-size:16px;width:5%">@format_currency($discount)</td> 
                            @elseif($dis != 0)
                                <td style="font-size:16px;width:5%">{{0}}</td> 
                                <td style="font-size:16px;width:5%">{{0}}</td> 
                            @endif
                            <td style="font-size:16px;width:5%">@format_currency($data->unit_price)</td>
                            <!--<td style="font-size:12px;font-weight: bold;">@format_currency($data->unit_price_inc_tax - $discount)</td>-->
                            <td style="font-size:16px;width:5%">@format_currency($data->unit_price*$data->quantity)</td>
        ...
        <table class="table" style="width:100%;font-size:11px;Lline-height:10px; margin-top: 10px;font-weight: bold; ">
            <thead>
                <tr>
                    <td>@lang("purchase.qty") : {{$count-1}}</td>
                </tr>
            </thead>
        </table>
        <table class="table" style="width:100%;font-size:11px;Lline-height:10px; margin-top: 10px;font-weight: bold; "  dir="ltr"    dir="ltr" >
            <tbody>
                <tr>
                ...
                <tr>
                    ...
                    <td style="border:0px solid black;color:transparent">{{"&nbsp;&nbsp;&nbsp;&nbsp;"}}</td>
                    <td>
                        Tax ({{ $invoice->tax->name  }} %):	 (+)  @format_currency( $invoice->tax_amount )
                    </td>
                </tr>
                <tr>
                    <td style="border:0px solid black;color:transparent">{{"&nbsp;&nbsp;&nbsp;&nbsp;"}}</td>
                    ...
                    <td style="border:0px solid black;color:transparent">{{"&nbsp;&nbsp;&nbsp;&nbsp;"}}</td>
                    <td>
                        Total:	  @format_currency( $invoice->final_total )
                    </td>
                </tr>
            </tbody>
        </table>
        <table class="table" style="width:100%;font-size:12px;margin-top: 30px;margin-bottom: 100px;text-align:left;border: 1px solid #BCBAB9;"  dir="ltr" >
            <tbody>
                <tr>
                    <td>
                        @php
        ...
        <table class="table" style="width:100%; font-size:12px; position:ablsolute;bottom:0px; text-align:left;border: 1px solid #BCBAB9;"  dir="ltr" >
            <tbody>
                <tr>
                    <td style="width:100%;border:3px solid black !important;">
                        {{"foooter"}}
                    </td>
                </tr>
            </tbody>
