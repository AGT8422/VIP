				...
				<tbody>
					@if($action == 'edit')
						...
						@foreach($combo_variations as $combo_variation)
							@php
								$list_of_prices_in_unit = \App\Product::getProductPrices($combo_variation['variation']->product->id);
                			...
                			@include('product.partials.combo_product_entry_row', 
								[
									...
									'list_price'             => isset($combo_variation['list_price'])?$combo_variation['list_price']:null,
								]
               				)
                		@endforeach
            		@endif
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		setTimeout(() => {	
			...
			function recalculate_the_row(row,item,qty_change=null,net_price=null){
				
				var purchase_price            = parseFloat(row.find('input.purchase_price').val());
				var base_unit_cost            = purchase_price;
				var prices                    = row.find('.list_price');
				var quantity                  = __read_number(row.find('input.quantity'), false);
				var multiplier                = __getUnitMultiplier(row);
				...
				cp_element                    = row.find('input.purchase_price')
				...
				html = "";
				global_html = "";
				list_price = JSON.stringify(row.find('.list_price').data("prices"));
				list_of    = JSON.parse(list_price);
				for(i in list_of){
				
					if(i == item.find(':selected').val()){
				...
				var item_level_purchase_price = quantity *  unit_cost;
				row.find('span.purchase_price_text').text(unit_cost);
				row.find('span.item_level_purchase_price').text(item_level_purchase_price);
				row.find('input.item_level_purchase_price').val(item_level_purchase_price);
			...
			function ready_functions(){
				...
				$(document).on('change', 'input.quantity', function(){
					...
					var row  = $(this).closest('tr');
					var item = row.find("select.sub_unit");
					var price = parseFloat(
									$(this)
									.find(':selected')
									.data('price')
								); 
					var new_row = row.find("input.quantity");
					if(isNaN(price) ){	
						row.find('select.list_price').children().each(function(){ 
					...
					recalculate_the_row(row,item,1,final_price);
				});

				$(document).on('change', 'select.sub_unit', function(){
					var item       = $(this);
					var row        = $(this).closest('tr');
					recalculate_the_row(row,item);
				});
				...
				$(document).on('change', 'table.combo_product_table select.list_price', function() {
					var row   = $(this).closest('tr') ;
					var price = parseFloat(
									$(this)
									.find(':selected')
