...
<tr class="row_variable_for_index">
    ...
    <td>
        <button type="button" data-ks="0"   data-variation_index="{{$variation_index}}"  data-value_index="{{$value_index}}" class="btn btn-danger btn-xs remove_variation_value_row">-</button>
        <button type="button" data-ks="0"  data-variation_index="{{$variation_index}}"  data-value_index="{{$value_index}}"  class="btn btn-primary btn-xs view_list_price" style="margin-top: 10px;"><i  style="font-size: 12px">&#36;</i></button>
        <input  type="hidden" class="variation_row_index" value="{{$value_index}}">
    </td>
</tr>
@include('product.partials.variable_list_price',[ "variation_index" => $variation_index , "value_index" => $value_index ,'ks'=>$ks,'units_main'=>$units_main,'other_unit'=>$other_unit , 'name'=>$name])
