                          ...
                          <table class="table table-condensed bg-black">
                             <thead>
                                <tr>
                                   ...
                                   <th>@lang("recieved.delivered_qty")</th>
                             ...
                             <tbody>
                              ...
                              @foreach ($items as $item)
                                 @if(!in_array($item->transaction->id,$array_))
                                    ...
                                    @php
                                       $total_rev = \App\Models\DeliveredPrevious::where("transaction_id",$item->transaction->id)->where("product_id",$item->product->id)->sum("current_qty");
                                 ...
                                 @if(!in_array($item->transaction->id,$array))
                                    ...
                                    <tr style="background:#f1f1f1; color:black">
                                       <td>
                                          ...
                                          data-href="{{action('SellController@show', [$item->transaction->id])}}" data-container=".view_modal">{{ $item->transaction?$item->transaction->invoice_no:'---' }}</button>  
                                          @else
                                             {{$item->transaction?$item->transaction->invoice_no:'---'}}
                                          @endif
                                       </td>
