...
<section class="content">
	{!! Form::open(['url' => action('PurchaseReturnController@store'), 'method' => 'post', 'id' => 'purchase_return_form' ,'files' => true ]) !!}
	...
	@component('components.widget', ['class' => 'box-primary', 'title' => __('lang_v1.parent_purchase')])
		<div class="row">
			...
			<div class="col-sm-4">
			  {{ trans('home.Cost Center') }} : {!! ($purchase->cost_center)?$purchase->cost_center->name:' ' !!}
			</div>
			<div class="col-sm-3">
	...
	@component('components.widget', ['class' => 'box-primary'])
			...
			<div class="col-sm-4">
				<div class="form-group">
					<div class="multi-input">
					  ...
					  <br/>
 					   {!! Form::select('currency_id', $currencies, ($purchase->return_parent)?$purchase->return_parent->currency_id:null, ['class' => 'form-control width-60 currency_id  select2', 'placeholder' => __('messages.please_select') ]); !!}
					  {!! Form::text('currency_id_amount', ($purchase->return_parent)?$purchase->return_parent->exchange_price:null, ['class' => 'form-control width-40 pull-right currency_id_amount'   ]); !!}
					</div>
				</div>
			</div>
			<div class="col-sm-6">
				<div class="form-group">
					...
					<input type="hidden" name="cost_center_id" value="{{$purchase->cost_center_id}}">
				...
				<table class="table bg-gray" id="purchase_return_table">
		          	<thead>
			            <tr class="bg-green">
							...
							<th @if(  $purchase->return_parent )@if($purchase->return_parent->exchange_price > 0) class="curr_column br_dis  cur_check  " @else class="curr_column br_dis  cur_check hide" @endif @else class="curr_column br_dis  cur_check hide" @endif>@lang( 'lang_v1.Cost before without Tax' ) @if($purchase->return_parent ) @if($purchase->return_parent->exchange_price > 0) {{  $purchase->return_parent->currency->symbol }} @endif @endif</th>
			        ...
			        <tbody>
			          	...
			          	@php
							...
							$price_item    = \App\Product::product_cost_purchase($purchase_line->product_id,$purchase_line->transaction_id);
			            ...
			            <tr>
			              	...
			              	<td><input  name="products[{{$purchase_line->id}}][unit_price_]"  class="form-control unit_price_" value="{{  $price_item }}">  </td>
			              	<td @if(  $purchase->return_parent ) @if(  $purchase->return_parent->exchange_price > 0   ) class="   cur_check  " @else class=" cur_check hide" @endif @else class=" cur_check hide" @endif>
								<input   name="products[{{$loop->index}}][unit_price_before_dis_exc_new_currency]" class="form-control unit_price_before_dis_exc_new_currency"  value="{{  (  $purchase->return_parent )? ( ( $purchase->return_parent->exchange_price > 0   )?round($price_item / $purchase->return_parent->exchange_price,4):0):0}}">
							</td>
							<td><span class="display_currency" data-is_quantity="true" data-currency_symbol="false">{{ $purchase_line->quantity }}</span> {{$unit_name}}</td>
			              	...
			              	<td>
					            ...
					            @if($purchase_line->product->enable_stock) 
			              		...
			              		@endif
					            >
					            {{-- <input type="hidden" class="unit_price" value="{{@num_format($price_item)}}"> --}}
			              	</td>
			              	<td>
		...
		<div class="row">
			...
			<div class="col-sm-12 text-right">
				<strong @if(  $purchase->return_parent ) @if($purchase->return_parent->exchange_price > 0 ) class="  cur_symbol" @else class="hide cur_symbol" @endif  @else class="hide cur_symbol" @endif>@lang('purchase.purchase_pay') @if($purchase->return_parent) @if($purchase->return_parent->exchange_price > 0 ) {{$purchase->return_parent->currency->symbol}} @endif @endif : </strong>&nbsp;
 				<span id="total_subtotal_cur" @if(  $purchase->return_parent )  @if($purchase->return_parent->exchange_price > 0 ) class="display_currency    " @else class="display_currency hide" @endif  @else class="display_currency hide" @endif ></span>
				<!-- This is total before purchase tax-->				
				<b>&nbsp;&nbsp;&nbsp;&nbsp;</b>
				<input type="hidden" id="total_subtotal_input_cur" value=0  name="total_before_tax_cur">
				<strong>@lang('purchase.purchase_pay'): </strong>&nbsp;
		...
		<div class="row">
			...
			<div class="col-sm-4 text-right">
				<div class="form-group">
					@php 
 						$returned = \App\Transaction::where("return_parent_id",$purchase->id)->first(); 
					@endphp
					@if($returned != null)
			...
			<div class="col-sm-4  text-right ">
				...
				@endphp
 				...
 				<span class="text-right"><strong>Discount Amount : </strong> &nbsp;<input style="width:30%;float:right" class="form-control " id="discount_" value="{{0}}"></span>
 				<h5>&nbsp;</h5>
			 
				<b   @if($purchase->return_parent) @if($purchase->return_parent->exchange_price > 0 )  class="i_curr  " @else class="i_curr hide" @endif @else class="i_curr hide" @endif>@lang( 'purchase.discount' ) @if($purchase->return_parent) @if($purchase->return_parent->exchange_price > 0 ) {{$purchase->return_parent->currency->symbol}} @endif @endif : (-) </b> 
				<span id="discount_calculated_amount_cur" @if($purchase->return_parent) @if($purchase->return_parent->exchange_price > 0 )    class="display_currency"  @else  class=" display_currency hide" @endif @else  class=" display_currency hide" @endif>{{ ($purchase->return_parent)?(($purchase->return_parent->exchange_price > 0 )?round($dis_amount_r / $purchase->return_parent->exchange_price):0):0}}</span>
				<b>&nbsp;&nbsp;&nbsp;&nbsp;</b>
				<b>@lang( 'purchase.discount' ):</b> (-) 
				<span id="dis_final" class="display_currency">{{$dis_amount_r}}</span>
  				<h5>&nbsp;</h5>
  				<h5>&nbsp;</h5>
				<b @if($purchase->return_parent) @if($purchase->return_parent->exchange_price > 0 )  class="o_curr "  @else class="o_curr hide" @endif @else class="o_curr hide" @endif >@lang('purchase.purchase_total_'): </b>
				<span id="grand_total_cur" @if($purchase->return_parent) @if($purchase->return_parent->exchange_price > 0 ) class="display_currency  " @else class="display_currency hide" @endif @else class="display_currency hide" @endif  >0</span>
				<b>&nbsp;&nbsp;&nbsp;&nbsp;</b>
				<b>@lang( 'purchase.purchase_total_' ):</b> (-) 
				<span id="total_after_dis" class="display_currency">0</span>
			 
			 
 				<h5>&nbsp;</h5>
				<b @if($purchase->return_parent)  @if($purchase->return_parent->exchange_price > 0 )  class="t_curr  " @else class="t_curr hide" @endif  @else class="t_curr hide" @endif  >@lang( 'lang_v1.total_return_tax' ) @if($purchase->return_parent) @if($purchase->return_parent->exchange_price > 0 ) {{$purchase->return_parent->currency->symbol}} @endif @endif: (+)</b> 
				<span id="tax_calculated_amount_curr" @if($purchase->return_parent)  @if($purchase->return_parent->exchange_price > 0 )  class="display_currency  " @else class="display_currency hide"@endif @else class="display_currency hide"@endif >0</span></td>
				<b>&nbsp;&nbsp;&nbsp;&nbsp;</b>
				...
				{!! Form::hidden('old_dis',($returned)?$returned->discount_amount:0, ['id' => 'old_dis']); !!}
				{!! Form::hidden('last_discount',0, ['id' => 'last_discount']); !!}
				{!! Form::hidden('total_rt_purchase',0, ['id' => 'total_rt_purchase']); !!}
				{!! Form::hidden('sub_total_rt_purchase',0, ['id' => 'sub_total_rt_purchase']); !!}
		...
		<div class="row">
			<div class="col-sm-12 text-right">
				<h5>&nbsp;</h5>
				<strong @if($purchase->return_parent) @if($purchase->return_parent->exchange_price > 0 )  class="z_curr  " @else class="z_curr hide" @endif  @else class="z_curr hide" @endif >@lang('lang_v1.return_total') @if($purchase->return_parent) @if($purchase->return_parent->exchange_price > 0 ) {{$purchase->return_parent->currency->symbol}} @endif @endif: </strong>&nbsp;
				<span id="total_final_i_curr"  @if($purchase->return_parent) @if($purchase->return_parent->exchange_price > 0 )  class="display_currency  " @else class="display_currency hide"@endif  @else class="display_currency hide"@endif >0</span> 
				<b>&nbsp;&nbsp;&nbsp;&nbsp;</b>
...
<script type="text/javascript">
	$(document).ready( function(){
		...
		ready_discount();
		// });
		// $(document).on('change', 'input.return_qty,input#discount_,#discount_type,input.unit_price', function(){
		all_changes();
	});
				...
				$.ajax({
					...
					success:function(data){
						...
						$(".currency_id_amount").val(object.amount);
 						...
 						$(".o_curr").html( @json(__('purchase.purchase_total_')) + " " + object.symbol +" : "   );
	...
	$(document).on('change', 'input.return_qty,input#discount_,#discount_type,input.unit_price_', function(){
		update_purchase_return_total()
	});

	function ready_discount(){
		$('input#discount_').ready( function(){
 			update_purchase_return_total();
		});
	}
	...
	function update_purchase_return_total(){
		...
		$('table#purchase_return_table tbody tr').each( function(){
			...
			var unit_price = __read_number($(this).find('input.unit_price_'));
		...
		var currancy      = $(".currency_id_amount").val();
 		...
 		if(discount_type == "") {
			var discount = 0;
			var total_after_dis = net_return - discount ;
			if($('#old_dis').val() != 0){
				var disc = $('#old_dis').val();
				$('input#discount_').val(disc);
				$('#last_discount').val(disc);
				$('#last_dis').val(disc);
				$('#old_dis').val(dis)

			}else{
				$('span#dis_final').text(__currency_trans_from_en(discount, true));
				if(currancy != "" && currancy != null){
					$('#discount_calculated_amount_cur').text( (discount/currancy).toFixed(4));
				}
				$('#last_discount').val(discount);
				$('#last_dis').val(dis);
				$('#old_dis').val(dis)

			}
 		}else if(discount_type == "fixed_before_vat") {
			var discount  =  $('input#discount_').val();
			var total_after_dis = net_return - discount ;
			if($('#old_dis').val() != 0){
				var disc = $('#old_dis').val();
				$('input#discount_').val(disc);
			...
			var dis      =  $('input#discount_').val();
			var discount =  net_return*(parseFloat(dis)/(100));
			var total_after_dis = net_return - discount ;
			if($('#old_dis').val() != 0){
				var disc = $('#old_dis').val();
				$('input#discount_').val(disc);
		...
		var total_tax = __calculate_amount('percentage', tax_percent, total_after_dis);
		var net_return_inc_tax = total_tax + total_after_dis;
		$('input#tax_amount').val(total_tax);
		$('span#total_after_dis').text(__currency_trans_from_en(total_after_dis, true));
		...
		if(currancy != "" && currancy != null){
			if(currancy == 0){
					$('#total_subtotal_cur').text(0);
					$('#grand_total_cur').text(0);
					$('#tax_calculated_amount_curr').text(0);
					$('#total_final_i_curr').text(0);
			}else{
				$('#total_subtotal_cur').text((net_return/currancy).toFixed(4));
				$('#grand_total_cur').text((total_after_dis/currancy).toFixed(4));
		...
		$('#total_rt_purchase').val( net_return_inc_tax  );
		$('#sub_total_rt_purchase').val( net_return  );
