            ...
            <table class="table" id="expense_report_table">
                ...
                <tbody>
                    ...
                    @foreach($values as $key => $it)
                        <tr>
                            <td>{{$labels[$key] ?? __('report.others')}} @if($expenses[$key]->daily_payment_item) {{$str}} @endif {{($expenses[$key]->daily_payment_item)?$expenses[$key]->daily_payment_item->daily_payment->ref_no:""}}</td>
                            <td><span class="display_currency" data-currency_symbol="true">{{$it}}</span></td>
                        </tr>
                        @php
                            $total_expense += $it;
                        @endphp
