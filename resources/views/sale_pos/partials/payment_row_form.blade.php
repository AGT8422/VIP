<div class="row">
	...
	<div class="{{$col_class}}">
		<div class="form-group">
			...
			<div class="input-group">
				...
				@php
					$_payment_method = empty($payment_line['method']) && array_key_exists('other', $payment_types) ? 'other' : $payment_line['method'];
				@endphp
	...
	@if(!empty($accounts))
		<div class="{{$col_class}}" id="pay_os_account" >
			<div class="form-group @if($readonly) hide @endif">
				<?php
				       $keys =  array_keys($accounts);
					   if (isset(array_keys($accounts)[0])) {
						$k =  array_keys($accounts)[0];
					   }
				 ?>
				...
				<div class="input-group">
					...
					{!! Form::select("payment[$row_index][account_id]", $accounts, !empty($payment_line['account_id']) ? $payment_line['account_id'] : $k ,
					 ['required','class' =>  'form-control select2 account-dropdown', 'id' => !$readonly ? "account_$row_index" : "account_advance_$row_index", 'style' => 'width:100%;', 'disabled' => $readonly]); !!}
				</div>
			</div>
		</div>
	@endif
	<div class="clearfix"></div>
		@include('sale_pos.partials.payment_type_details',['cheque_type'=>($cheque_type)??1])
	<div class="col-md-12">
		<div class="form-group">
