	...
	<div class="col-xs-12 text-center">
		...
		<p style="width: 100% !important" class="word-wrap">
			<span class="pull-left text-left word-wrap">
                ...
                <br>
				<b>{{__("sale.project_no")}}</b>
			    {!! "#" . $receipt_details->project_no !!} 
...
<div class="row">
	<div class="col-xs-12">
		<br/>
		@php
    		$currency = \App\Currency::find($receipt_details->useful_id); 
				...
				@forelse($receipt_details->lines as $line)
					<tr>
						<td>
                            ...
                            @if(!empty($line['warranty_name'])) <br><small>{{$line['warranty_name']}} </small>@endif @if(!empty($line['warranty_exp_date'])) <small>   </small>@endif
                            @if(!empty($line['warranty_description'])) <small> {{$line['warranty_description'] ?? ''}}</small>@endif
                        </td>
						<td class="text-right">{{$line['quantity']}}{{$line['units']}} </td>
						<td class="text-right">{{$line['unit_price_inc_tax']}} {{  " " . $currency_symbol  }}</td>
						@if(!empty($receipt_details->item_discount_label))
							<td class="text-right">
								{{$line['line_discount'] ?? '0.00'}}
							</td>
						@endif
						<td class="text-right">{{$line['line_total']}}  {{  " " . $currency_symbol  }}</td>
					</tr>
