...
@if($quantity != 0)
    <tr class="product_row line_sorting">
                ...
                @if(!empty($product->lot_numbers))
                    <select class="form-control lot_number" name="products[{{$row_index}}][lot_no_line_id]">
                        ...
                        @foreach($product->lot_numbers as $lot_number)
                            ...
                            @endphp
                            <option value="{{$lot_number->purchase_line_id}}" data-qty_available="{{$quantity}}" data-msg-max="@lang('lang_v1.quantity_error_msg_in_lot', ['qty'=> $quantity, 'unit' => $product->unit  ])" {{$selected}}>@if(!empty($lot_number->lot_number) && $lot_enabled == 1){{$lot_number->lot_number}} @endif @if($lot_enabled == 1 && $exp_enabled == 1) - @endif @if($exp_enabled == 1 && !empty($lot_number->exp_date)) @lang('product.exp_date'): {{@format_date($lot_number->exp_date)}} @endif {{$expiry_text}}</option>
                        @endforeach
                    </select>
                @endif
            @endif
        ...
        <td>
            ...
            data-rule-required="true" data-msg-required="@lang('validation.custom-messages.this_field_is_required')" @if($product->enable_stock) data-rule-max-value="{{$quantity}}" data-msg-max-value="@lang('validation.custom-messages.quantity_not_available', ['qty'=> $quantity, 'unit' => $product->unit  ])"
            data-qty_available="{{$product->qty_available}}" 
            data-msg_max_default="@lang('validation.custom-messages.quantity_not_available', ['qty'=> $quantity, 'unit' => $product->unit  ])"
            @endif >
            {{$product->unit}}
        ...
        <td>
            ...
            @lang("lang_v1.available_qty") :  {{$quantity}} 
            <input type="text" readonly name="products[{{$row_index}}][price]" class=" hide form-control product_line_total" value="{{@num_format($product->quantity_ordered)}}">
        </td>
...
@else
    {{-- <div id="out" style="text-align:center;">Out Of Stock</div> --}}
