			...
			<div class="row">
				...
				<div class="col-sm-6">
					<div class="form-group">
						...
						{!! Form::select('location_id', $mainstore_categories, null, ['class' => 'form-control select2', 'placeholder' => __('messages.please_select'), 'required', 'id' => 'location_id']); !!}
					</div>
				</div>
				...
				<div class="col-sm-6">
					<div class="form-group">
						...
						{!! Form::select('transfer_location_id', $mainstore_categories, null, ['class' => 'form-control select2', 'placeholder' => __('messages.please_select'), 'required', 'id' => 'transfer_location_id']); !!}
					</div>
				</div>
				<div class="col-sm-12">
					<div class="form-group hide">
					 
						{!! Form::select('location_id1', $business_locations ,$default_location, ['class' => '  form-control select2  ' , 'id' => 'location_id1']); !!}
					</div>
				</div>
			...
			<div class="row">
				<div class="col-sm-10 col-sm-offset-1">
					...
					id="stock_adjustment_product_table">
						...
						<tfoot>
							<tr class="hide text-center"><td colspan="2"></td><td><div class="pull-right"><b>@lang('stock_adjustment.total_amount'):</b> <span id="total_adjustment_">0.00</span></div></td></tr>
						</tfoot>
					</table>
					</div>
				</div>
	...
	<script type="text/javascript">
		...
		$("#transfer_location_id").on("change",function(){
			if($(this).val() == $("#location_id").val()){
				toastr.error("should change store");
				$("#save_stock_transfer").prop('disabled', true);
					
			}else{
				$("#save_stock_transfer").prop('disabled', false);
			}
		})
		$("#location_id").on("change",function(){
			if($(this).val() == $("#transfer_location_id").val()){
				toastr.error("should change store");
				$("#save_stock_transfer").prop('disabled', true);
